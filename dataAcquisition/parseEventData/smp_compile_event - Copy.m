...
%                                br2_channel, br2_protocol, beacon_check, ...
...
%     if nargin < 16, beacon_check   = false;               end
%     if nargin < 17, unique_fp      = false;               end
%     if nargin < 18, show_report    = false;               end
%     if nargin < 19, concat_csv_dir = '';                  end
%     if nargin < 20, all_laps       = false;               end
%     if nargin < 21, load_all_ch    = false;               end
%     MYLAPS_CH_DEFAULT        = 'MyLaps_X2TRA_DeviceShortId';
% 
%     % ------------------------------------------------------------------
%     %  lap_slicer options — built via smp_lap_opts_build so this always
%     %  matches smp_recompute_vch. min_lt/max_lt are already resolved by
%     %  the caller (from smp_season_get), so they're passed straight
%     %  through via opts.min_lap_time/.max_lap_time, which the helper
%     %  uses as-is instead of re-deriving them.
%     % ------------------------------------------------------------------
%     lap_opts_in.min_lap_time   = min_lt;
%     lap_opts_in.max_lap_time   = max_lt;
%     lap_opts_in.detect_pitlane = detect_pitlane;
%     lap_opts_in.fcy_channel    = fcy_channel;
%     lap_opts_in.br2_channel    = br2_channel;
%     lap_opts_in.br2_protocol   = br2_protocol;
%     lap_opts_in.beacon_check   = beacon_check;
%     lap_opts_in.verbose        = false;
%     lap_opts = smp_lap_opts_build([], '', lap_opts_in);
% 
%     lap_opts.mylaps_channel  = MYLAPS_CH_DEFAULT;
%     lap_opts.save_beacon_pdf   = true;
%     lap_opts.beacon_pdf_dir    = fullfile('/Volumes/JHU/reports/lap_slicer', 'beacon_checks');
%     lap_opts.beacon_check_pause = false;   % set true if you want it to pause on every driver
%     lap_opts.beacon_pdf_name    = 'beacon_checks_combined.pdf';
% 
...
%         % ---- Lap slice ----
%         try
%             lap_opts.beacon_check_label = sprintf('%s — %s — #%s — %s', ...
%                 grp.team_acronym, grp.driver, grp.session);
...
%         session = smp_custom_channels(session, 'manufacturer', session.info.manufacturer, 'driver', session.info.driver);
%         if ~load_all
%             session = filter_channels(session, channels_to_extract);
%         end
%         return;
...
%         t0 = tic;
%         s = smp_custom_channels(s, 'manufacturer', s.info.manufacturer, 'driver', s.info.driver);
...
%     saveCache     = get_opt(opts, 'saveCache',       true);
    ...
    if ~isempty(pre_groups)
        ...
        cache = process_stream(cache, pre_groups, channels_to_extract, ...
                               ...
                               unique_fp, show_report, concat_csv_dir, all_laps, load_all_ch, T_gated);
        % saveCache is intentionally skipped for worker partial caches
        return;
    end
...
function cache = process_stream(cache, groups, channels_to_extract, ...
    ...
    for g = 1:n_groups
        ...
        try
            session = load_and_concat(grp.files, ch_extract_full, verbose, ...
                                      unique_fp, show_report, grp.key, concat_csv_dir, load_all_ch, driver_map, T_gated);
        catch ME
            fprintf('  [ERROR] Load failed: %s\n', ME.message);
            cache = add_failed_entries(cache, grp, ME.message);
            continue;
...
function session = load_and_concat(files, channels_to_extract, verbose, unique_fp, show_report, label, csv_out_dir, load_all, driver_map, T_gated)
    if nargin < 4,  unique_fp    = false;   end
    if nargin < 5,  show_report  = false;   end
    if nargin < 6,  label        = '';      end
    if nargin < 7,  csv_out_dir  = '';      end
    ...
    for f = 1:numel(files)
        ...
        channels_to_extract = union(channels_to_extract, [vch_names(:); gated_names(:)]);
        fprintf('  smp_custom_channels + smp_gated_channels: %.2fs\n', toc(t0));
        if ~load_all
            t0 = tic;
            s = filter_channels(s, channels_to_extract);
            fprintf('  filter_channels: %.2fs\n', toc(t0));
