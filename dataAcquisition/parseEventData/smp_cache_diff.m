        ...
        for f = 1:numel(team.files)
            ...
            if isKey(path_map, fpath)
                ...
                if changed || failed
                    ...
                    % Update path_map in-place: remove this key and
                    % decrement all indices that shifted due to row removal
                    path_map.remove(fpath);
                    stale_keys = keys(path_map);
                    for ki = 1:numel(stale_keys)
                        if path_map(stale_keys{ki}) > row
                            path_map(stale_keys{ki}) = path_map(stale_keys{ki}) - 1;
                        end
                    end
                end
            else
