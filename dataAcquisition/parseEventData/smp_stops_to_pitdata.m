function [pitData, pit_summary] = smp_stops_to_pitdata(stops, SMP, driver_map)
...
%
%   pit_summary - table, one row per stop, sorted by Car then Lap. Columns:
%                   Car        string  car number (from driver_map NUM column)
%                   Lap        double  pit-in lap number
%                   NumTyres   double  number of tyres changed
%                   StopNumber double  sequential stop count per car (1 = first)
...
for t = 1:numel(teams)
    ...
    for r = 1:numel(stops.(tm))
       ...
       % ── Resolve driver & manufacturer from SMP metadata ──────────────
        ...
        % ── Resolve car number for pit_summary ───────────────────────────
        car_num = resolve_car_num(tla, driver_map);
        ...
        for s = 1:n
            ...
            % Accumulate for pit_summary
            sum_car{end+1}    = car_num;     %#ok
            sum_lap(end+1)    = st.pit_lap;  %#ok
            sum_ntyres(end+1) = st.tyres_changed; %#ok
        end
...
% ── Build pit_summary ────────────────────────────────────────────────────
if isempty(sum_lap)
    pit_summary = table('Size', [0 4], ...
        'VariableTypes', {'string','double','double','double'}, ...
        'VariableNames', {'Car','Lap','NumTyres','StopNumber'});
else
    Car      = string(sum_car(:));
    Lap      = double(sum_lap(:));
    NumTyres = double(sum_ntyres(:));

    % Sort by Car then Lap
    tmp      = table(Car, Lap, NumTyres);
    tmp      = sortrows(tmp, {'Car','Lap'}, {'ascend','ascend'});
    Car      = tmp.Car;
    Lap      = tmp.Lap;
    NumTyres = tmp.NumTyres;

    % Assign sequential stop number per car
    n_rows     = height(tmp);
    StopNumber = zeros(n_rows, 1);
    prev_car   = '';
    cnt        = 0;
    for i = 1:n_rows
        if ~strcmp(char(Car(i)), prev_car)
            cnt      = 1;
            prev_car = char(Car(i));
        else
            cnt = cnt + 1;
    ...
    pit_summary = table(Car, Lap, NumTyres, StopNumber, ...
        'VariableNames', {'Car','Lap','NumTyres','StopNumber'});
end
...
function num = resolve_car_num(tla, driver_map)
% Look up car number string from driver_map by TLA. Falls back to TLA.
    num = tla;
    if isempty(driver_map) || ~isstruct(driver_map), return; end
    keys = fieldnames(driver_map);
    for k = 1:numel(keys)
        entry = driver_map.(keys{k});
        if isfield(entry, 'tla') && strcmpi(entry.tla, tla)
