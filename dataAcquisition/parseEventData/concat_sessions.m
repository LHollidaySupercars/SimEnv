...
% 
%     % ---- Engine xcorr parameters (Method 5) ----
%     RPM_CANDIDATES = {'Engine_Speed', 'Engine_RPM', 'RPM'};
%     XCORR_THRESH   = 0.97;  % min Pearson r at best-lag window to call duplicate
% 
...
%     %   2. Start+end fingerprint  — exact duplicate only
%     %   3. Distributed sample     — equal-length exact duplicate
%     %   4. Engine_Speed xcorr     — interior subset detection (no GPS needed)
...
%                     end
%                 end
%                 % ---- LAP-TIME GUARD: only proceed to xcorr if lap times
%                 % confirm S is a temporal subset of R.
%                 % Two similar stints at the same circuit will share RPM
%                 % structure but will NOT share the same lap-time sequence.
%                 % Requiring 3+ matching laps in a contiguous block prevents
%                 % false-positive xcorr drops. ----
%                 [lap_confirms_subset, lap_info] = lap_time_subset(sessions{s}, sessions{r});
%                 if verbose
%                     fprintf('  S%d vs R%d | Lap-guard: subset=%d  info=%s\n', ...
%                         s, r, lap_confirms_subset, lap_info);
%                 end
%                 if ~lap_confirms_subset
%                     if verbose
%                         fprintf('  S%d vs R%d | Lap-guard BLOCKED xcorr — not dropping.\n', s, r);
%                     end
%                     continue;
%                 end
%                 % ---- METHOD 5: Engine_Speed xcorr (interior subset detection) ----
%                 % Slides S over R via cross-correlation on the raw RPM channel.
%                 % Finds the best-fit offset then computes windowed Pearson r.
%                 % Works for interior subsets regardless of GPS quality or file length.
%                 if ~is_dup
%                     [is_dup_xc, xcorr_r, lag_s] = engine_xcorr_match( ...
%                         sessions{s}, sessions{r}, RPM_CANDIDATES, XCORR_THRESH);
%                     if verbose
%                         fprintf('  S%d vs R%d | M5-Xcorr: r=%.3f  lag=%.1fs  is_dup=%d\n', ...
%                             s, r, xcorr_r, lag_s, is_dup_xc);
%                     end
%                     if is_dup_xc
%                         is_dup = true;
%                         method = sprintf('engine xcorr (r=%.2f, lag=%.1fs)', xcorr_r, lag_s);
%                     end
%                 end
% 
...
%             fn = ch_fields{c};
%             if isfield(merged, fn) && isfield(merged.(fn), 'data') && ...
%                isfield(merged.(fn), 'time') && numel(merged.(fn).time) > 1
...
%     LAP_TOL   = 0.1;   % seconds — covers ±0.05 s per boundary (0.1 s resolution)
%     is_subset = false;
%     info      = '';
% 
...
%     if numel(laps_s) < 3, return; end               % need at least 3 laps to be meaningful
% 
...
% 
% % -------------------------------------------------------------------------
% function [is_dup, peak_r, lag_s] = engine_xcorr_match(sess_s, sess_r, candidates, thresh)
% % ENGINE_XCORR_MATCH  Detect if sess_s is a subset of sess_r via RPM xcorr.
% %
% % Same logger, same channel, same sample rate — no resampling needed.
% % xcorr(d_r, d_s) slides S over R to find the best-fit offset.  At that
% % offset a windowed Pearson r is computed; if >= thresh, S is a duplicate.
% %
% % xcorr convention: peak at lag L means d_r[L+1 : L+n_s] best matches d_s,
% % i.e. S starts L samples into R.
% 
%     is_dup = false;
%     peak_r = 0;
%     lag_s  = 0;
...
%     n_r = numel(d_r);
% 
%     % S must be no longer than R to be a subset
%     if n_s > n_r, return; end
% 
%     % Remove mean; require meaningful RPM variance (engine running + varying)
%     d_s = d_s - mean(d_s);
%     d_r = d_r - mean(d_r);
%     if std(d_s) < 100 || std(d_r) < 100, return; end
% 
%     % Step 1: find best lag via xcorr
%     [xc, lags]    = xcorr(d_r, d_s);
%     [~, peak_idx] = max(xc);
%     lag_samp      = lags(peak_idx);
% 
%     % Step 2: extract the window in R at that lag
%     i_start = lag_samp + 1;        % xcorr(d_r,d_s): peak lag = offset of S in R (1-based)
%     i_end   = i_start + n_s - 1;
%     if i_start < 1 || i_end > n_r, return; end
% 
%     window_r = d_r(i_start:i_end);
% 
%     % Step 3: Pearson r between window and S
%     std_w = std(window_r);
%     std_s = std(d_s);
%     if std_w < eps || std_s < eps, return; end
%     peak_r = mean((window_r - mean(window_r)) .* (d_s - mean(d_s))) / (std_w * std_s);
% 
%     % Step 4: lag in seconds from S's time axis
%     dt_s = median(diff(sess_s.(fn_s).time(:)));
%     if ~isfinite(dt_s) || dt_s <= 0, dt_s = 0.01; end
%     lag_s = lag_samp * dt_s;
% 
%     is_dup = peak_r >= thresh;
% end
function [merged, report] = concat_sessions(sessions, opts)
% CONCAT_SESSIONS  Concatenate a cell array of session structs along their
%                  time axes.
...
% laps across the file boundary.
%
% PERFORMANCE NOTE (rewritten): the original appended each stint via
% [merged.(fn).data; s2.(fn).data] inside the stint loop, which reallocates
% and copies the ENTIRE accumulated array on every iteration — O(k^2) total
% data movement for k stints. This version splits the work into two passes:
%   Pass 1 — sequential bookkeeping only (t_offset, lap_offset per stint).
%            Cheap: scalars and small median/max calls, no large array copies.
%   Pass 2 — one vertcat per channel across all stints at once.
%            O(k) total data movement instead of O(k^2).
% Duplicate/subset detection and lap renumbering logic are unchanged from
% the original — only WHEN the large arrays get concatenated has changed.
%
% INPUT
%   sessions  - {1 x N} cell array of session structs.
%               Each struct has fields named after channels;
...
%     (near-zero variance) fingerprints are never considered a match.
    ...
    % ===================================================================
    %  APPEND STINTS — two-pass rewrite (was: single grow-in-place loop)
    % ===================================================================
    merged    = sessions{1};
    ch_fields = fieldnames(merged);
    ...
    if n_sess == 1
        return;   % nothing to concatenate, merged is already correct
    end

    % ---- Pass 1: sequential bookkeeping only (t_offset, lap_offset per stint) ----
    % Mirrors the original loop's t_last/dt_gap/lap_offset formulas exactly,
    % but computes them WITHOUT mutating merged.(fn).data/.time — those big
    % arrays are only touched once, in Pass 2.
    %
    % dt_gap approximation note: the original recomputed dt_min from
    % merged.(fn).time AFTER each prior append (i.e. from the growing
    % cumulative array). Because sample rate is constant within a channel
    % for a given stint, median(diff(...)) on the cumulative array is
    % dominated by intra-stint spacing regardless of how many stints have
    % already been folded in — the one large inter-stint gap sample is
    % negligible against a median over thousands of samples. This pass
    % approximates that same value using each channel's own time array
    % directly (stint 1 for dt reference, consistent with the original's
    % steady-state behaviour). Verify against the original on a real
    % multi-stint race group before trusting on a live compile.
    t_offsets   = zeros(n_sess, 1);   % t_offsets(1) unused — stint 1 is the base
    lap_offsets = zeros(n_sess, 1);   % same

    run_end_time = struct();   % run_end_time.(fn) = current end time for that channel
    run_lap_max  = NaN;        % running max Lap_Number across appended stints so far

    for c = 1:numel(ch_fields)
        fn = ch_fields{c};
        if isfield(merged, fn) && isfield(merged.(fn), 'time') && numel(merged.(fn).time) > 0
            run_end_time.(fn) = merged.(fn).time(end);
        end
    end
    if isfield(merged, 'Lap_Number') && isfield(merged.Lap_Number, 'data') && ~isempty(merged.Lap_Number.data)
        run_lap_max = max(round(merged.Lap_Number.data));
    end

    for s = 2:n_sess
        s2 = sessions{s};

        % ---- t_offset: same t_last/dt_gap logic as the original ----
        t_last = -Inf;
        dt_min = Inf;
        dt_gap = 0.02;   % fallback: 50 Hz, matches original default
        for c = 1:numel(ch_fields)
            fn = ch_fields{c};
            if isfield(run_end_time, fn)
                t_last = max(t_last, run_end_time.(fn));
            end
            if isfield(merged, fn) && isfield(merged.(fn), 'data') && ...
               isfield(merged.(fn), 'time') && numel(merged.(fn).time) > 1
                dt_c = median(diff(merged.(fn).time));
        ...
        t_offsets(s) = t_last + dt_gap;

        % ---- lap_offset: identical formula to the original ----
        if ~isnan(run_lap_max) && isfield(s2, 'Lap_Number') && isfield(s2.Lap_Number, 'data') && ...
           ~isempty(s2.Lap_Number.data)
            lap_min2       = min(round(s2.Lap_Number.data));
            lap_offsets(s) = run_lap_max + 1 - lap_min2;
        end

        % ---- Advance running state as if this stint had been appended ----
        for c = 1:numel(ch_fields)
            fn = ch_fields{c};
            if isfield(s2, fn) && isfield(s2.(fn), 'time') && numel(s2.(fn).time) > 0
                run_end_time.(fn) = s2.(fn).time(end) + t_offsets(s);
            end
        end
        if isfield(s2, 'Lap_Number') && isfield(s2.Lap_Number, 'data') && ~isempty(s2.Lap_Number.data)
            new_max = max(round(s2.Lap_Number.data)) + lap_offsets(s);
            if isnan(run_lap_max) || new_max > run_lap_max
                run_lap_max = new_max;
            end
        end
    end

    % ---- Pass 2: actual data movement — one vertcat per channel, not k-1 ----
    for c = 1:numel(ch_fields)
        fn = ch_fields{c};
        if ~isfield(merged.(fn), 'data') || ~isfield(merged.(fn), 'time')
            continue;   % skip non-channel fields (e.g. .info) — matches original guard
        end

        data_chunks = cell(n_sess, 1);
        time_chunks = cell(n_sess, 1);
        data_chunks{1} = merged.(fn).data(:);
        time_chunks{1} = merged.(fn).time(:);
        n_valid = 1;

        for s = 2:n_sess
            s2 = sessions{s};
            if ~isfield(s2, fn), continue; end   % matches original ~isfield(s2, fn) guard

            n_valid = n_valid + 1;
            d = s2.(fn).data(:);

            % Same Lap_Number renumbering the original applied to s2 in place
            if strcmp(fn, 'Lap_Number') && lap_offsets(s) ~= 0
                d = d + lap_offsets(s);
            end

            data_chunks{n_valid} = d;
            time_chunks{n_valid} = s2.(fn).time(:) + t_offsets(s);
        end

        merged.(fn).data = vertcat(data_chunks{1:n_valid});
        merged.(fn).time = vertcat(time_chunks{1:n_valid});
    end
end
