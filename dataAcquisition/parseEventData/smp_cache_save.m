...
%             (original behaviour — backwards compatible).
%
%             If a session's data would exceed ~2GB, it is further split
%             into team-based parts: smp_cache_RA1_Part1.mat,
%             smp_cache_RA1_Part2.mat, etc. Teams are greedily bin-packed
%             by estimated in-memory size so each part stays under the
%             2GB threshold. Sessions that fit in one file keep the
%             original naming (no _PartN suffix) — no existing single-file
%             sessions are affected.
%
%             cache.laps (if present) is partitioned and saved alongside
%             stats/traces, keyed the same way by GroupKey.
%
% Usage:
%   smp_cache_save(top_level_dir, cache)                      % legacy
%   smp_cache_save(top_level_dir, cache, 'session')           % session, raw names
...
% ======================================================================= %
%  SESSION SPLIT — one .mat per session, auto-split into parts if >2GB
% ======================================================================= %
function save_by_session(top_level_dir, cache, alias)
    ...
    % Team/group field used both for stats/traces/laps field names and for
    % bin-packing sessions that exceed the size threshold.
    TEAM_FIELD = 'GroupKey';   % adjust here if teams are tracked under a different manifest column
    MAX_BYTES  = 2 * 1024^3;   % 2GB split threshold
    ...
    for s = 1:numel(raw_sessions)
        ...
        % ---- Partition stats, traces, and laps by group key ----
        part_stats  = struct();
        part_traces = struct();
        part_laps   = struct();
        ...
        if ismember('GroupKey', part_manifest.Properties.VariableNames)
            ...
            for k = 1:numel(gkeys)
                ...
                if has_laps_field && isfield(cache.laps, gk_vld)
                    part_laps.(gk_vld) = cache.laps.(gk_vld);
                end
            end
        else
            % No GroupKey — best effort match by session name in key string
            ...
            for k = 1:numel(all_keys)
                if contains(lower(all_keys{k}), lower(sess_safe))
                    ...
                    if has_laps_field && isfield(cache.laps, all_keys{k})
                        part_laps.(all_keys{k}) = cache.laps.(all_keys{k});
                    end
                end
            end
        end

        % ---- Bin-pack teams within this session by estimated size ----
        if ismember(TEAM_FIELD, part_manifest.Properties.VariableNames)
            tkeys = unique(string(part_manifest.(TEAM_FIELD)));
            tkeys = tkeys(strtrim(tkeys) ~= "");
            if isempty(tkeys), tkeys = "ALL"; end
        else
            tkeys = "ALL";
        end

        bins = {}; bin_sizes = []; cur_bin = {}; cur_size = 0;

        for k = 1:numel(tkeys)
            tk     = char(tkeys(k));
            tk_vld = matlab.lang.makeValidName(tk);
            sz = 0;
            if isfield(part_stats, tk_vld),  sz = sz + whos_size(part_stats.(tk_vld));  end
            if isfield(part_traces, tk_vld), sz = sz + whos_size(part_traces.(tk_vld)); end
            if isfield(part_laps, tk_vld),   sz = sz + whos_size(part_laps.(tk_vld));   end

            if cur_size + sz > MAX_BYTES && ~isempty(cur_bin)
                bins{end+1}     = cur_bin;  %#ok<AGROW>
                bin_sizes(end+1) = cur_size; %#ok<AGROW>
                cur_bin = {}; cur_size = 0;
            end
            cur_bin{end+1} = tk; %#ok<AGROW>
            cur_size = cur_size + sz;
        end
        if ~isempty(cur_bin)
            bins{end+1}     = cur_bin;
            bin_sizes(end+1) = cur_size;
        end
        if isempty(bins)
            % No team field / no stats matched — single part, whole session
            bins      = {cellstr(tkeys)};
            bin_sizes = 0;
        end
        ...
        for p = 1:n_parts
            if n_parts == 1
                ...
                cache_path = fullfile(top_level_dir, sprintf('smp_cache_%s_Part%d.mat', sess_safe, p));
            end

            keep_teams = bins{p};

            part_stats_p  = struct();
            part_traces_p = struct();
            part_laps_p   = struct();
            for k = 1:numel(keep_teams)
                tk_vld = matlab.lang.makeValidName(keep_teams{k});
                if isfield(part_stats, tk_vld)
                    part_stats_p.(tk_vld) = part_stats.(tk_vld);
                end
                if isfield(part_traces, tk_vld)
                    part_traces_p.(tk_vld) = part_traces.(tk_vld);
                end
                if isfield(part_laps, tk_vld)
                    part_laps_p.(tk_vld) = part_laps.(tk_vld);
                end
            end

            if n_parts > 1 && ismember(TEAM_FIELD, part_manifest.Properties.VariableNames)
                row_keep = ismember(string(part_manifest.(TEAM_FIELD)), string(keep_teams));
                manifest_p = part_manifest(row_keep, :);
            else
                manifest_p = part_manifest;
            end

            % ---- Assemble session cache ----
            sess_cache            = struct();
            sess_cache.manifest   = manifest_p;
            sess_cache.stats      = part_stats_p;
            sess_cache.traces     = part_traces_p;
            sess_cache.laps       = part_laps_p;
            sess_cache.mode       = cache.mode;
            sess_cache.save_mode  = 'session';
            sess_cache.canonical_session = canonical;   % store for load-side display
            ...
            % ---- Save ----
            if n_parts == 1
                fprintf('  [%d/%d] "%s" -> "%s"  (%d rows, %d stats, %d traces, %d laps)\n', ...
                    s, numel(raw_sessions), raw_sess, canonical, height(manifest_p), ...
                    numel(fieldnames(part_stats_p)), numel(fieldnames(part_traces_p)), numel(fieldnames(part_laps_p)));
            else
                fprintf('  [%d/%d] "%s" -> "%s" Part%d/%d  (~%.2fGB, %d rows, %d stats, %d traces, %d laps)\n', ...
                    s, numel(raw_sessions), raw_sess, canonical, p, n_parts, bin_sizes(p)/1024^3, ...
                    height(manifest_p), numel(fieldnames(part_stats_p)), numel(fieldnames(part_traces_p)), numel(fieldnames(part_laps_p)));
            end
...
% ======================================================================= %
%  Estimate in-memory size (bytes) of a variable via WHOS
% ======================================================================= %
