function smp_show_concat_report(report, label, file_names, sessions, merged)
...
%                 enables a post-concat result trace panel
    ...
    uitable('Parent', fig, ...
        'Data', tbl_data, ...
        'ColumnName', {'#', 'File', 'Status', 'Matched', 'Reason'}, ...
        'ColumnWidth', {28, 360, 70, 90, 390}, ...
        'Units', 'pixels', 'Position', [5 y 1050 TBL_H], ...
        'FontSize', 10, ...
        'BackgroundColor', [0.16 0.16 0.16; 0.20 0.20 0.20], ...
    ...
    if has_traces
        y = y - TRC_H;
        ax_t = axes('Parent', fig, 'Units', 'pixels', ...
    ...
    if has_merged
        ...
        ax_m = axes('Parent', fig, 'Units', 'pixels', ...
        ...
        if ~isempty(ch_m)
            t_m = merged.(ch_m).time(:);
            v_m = merged.(ch_m).data(:);
            if numel(t_m) > MAX_PTS
                idx = round(linspace(1, numel(t_m), MAX_PTS));
                t_m = t_m(idx);  v_m = v_m(idx);
            end
            plot(ax_m, t_m/60, v_m, '-', 'Color', [0.40 0.90 0.40], 'LineWidth', 1.6);
            u_m = '';
        ...
        title(ax_m, 'Merged result — post-concat time axis', ...
              'Color','w','FontSize',10);
    ...
    for si = 1:n
        ...
        y = y - ALIGN_H;
        ax_a = axes('Parent', fig, 'Units', 'pixels', ...
