...
%     if ~isfield(cache, 'laps'),      cache.laps      = struct(); end  % ← add here
...
%         % ---- Merge Laps ------
%         if isfield(sc, 'laps')
%             fns = fieldnames(sc.laps);
%             for k = 1:numel(fns)
%                 cache.laps.(fns{k}) = sc.laps.(fns{k});
%             end
%         end
...
% file) or 'session' (per-session files) mode, and loads accordingly.
%
% Session-mode files may be split into size-bounded parts by
% smp_cache_save (smp_cache_RA1_Part1.mat, smp_cache_RA1_Part2.mat, ...)
% when a session's data exceeds ~2GB. All parts belonging to the same
% session are detected and merged transparently — sessions that fit in
% a single file are unaffected (smp_cache_RA1.mat, no _PartN suffix).
%
% Usage:
%   cache = smp_cache_load(top_level_dir)
%       Legacy mode  : loads smp_cache.mat  (all sessions, same as before)
%       Session mode : loads ALL smp_cache_*.mat files (and all their
%                      parts) and merges them
%
%   cache = smp_cache_load(top_level_dir, {'RA1','RA2'})
%       Legacy mode  : loads smp_cache.mat  (filter applied after load —
%                      same RAM cost, but you only pay it once at compile time)
%       Session mode : loads ONLY smp_cache_RA1*.mat + smp_cache_RA2*.mat
%                      (including any _PartN files) — other sessions never
%                      touch RAM
%
%   cache = smp_cache_load(top_level_dir, {})
%       Loads all sessions regardless of mode (pass {} or omit arg).
...
% ======================================================================= %
%  LOAD SESSION FILES — merge selected smp_cache_*.mat files (incl. parts)
% ======================================================================= %
function cache = load_session_files(top_level_dir, session_files, session_filter)

    % Build list of (session_name, file_path) pairs from filenames.
    % Filename formats:
    %   smp_cache_<sess_safe>.mat            (single file, no split)
    %   smp_cache_<sess_safe>_Part<N>.mat    (size-split part)
    % Both forms resolve to the same session_name so all parts of a
    % session are grouped together automatically.
    file_sessions = cell(numel(session_files), 1);
    file_paths    = cell(numel(session_files), 1);

    for i = 1:numel(session_files)
        fname = session_files(i).name;
        tok = regexp(fname, '^smp_cache_(.+?)(?:_Part\d+)?\.mat$', 'tokens');
        if ~isempty(tok)
            file_sessions{i} = tok{1}{1};   % e.g. 'FP1', 'RA1'
        else
            file_sessions{i} = fname;
    ...
    if isempty(session_filter)
        to_load_idx = 1:numel(session_files);
        fprintf('Loading cache (session mode): all %d file(s) (sessions + parts)...\n', numel(session_files));
    else
        % Match session_filter against discovered session names
        to_load_idx = [];
        for f = 1:numel(file_sessions)
        ...
        fprintf('Loading cache (session mode): %d file(s) matching [%s]...\n', ...
            numel(to_load_idx), strjoin(session_filter, ', '));
    end
    ...
    for i = 1:numel(to_load_idx)
        ...
        fprintf('  [%d/%d] %s (%s) ... ', i, numel(to_load_idx), sess, fname_only);
        tic;
        loaded = load(fpath, 'sess_cache');
        sc     = loaded.sess_cache;
        t      = toc;
