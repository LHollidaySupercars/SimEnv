...
TOP_LEVEL_DIR     = '/Volumes/JHU/E06_DAR/_TeamData';

CHANNELS_FILE     = '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/channels/channels.xlsx';
EVENT_ALIAS_FILE  = '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/alias/eventAlias.xlsx';
DRIVER_ALIAS_FILE = '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/alias/driverAlias.xlsx';
%  PLOT_CONFIG_FILES  = {'/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/plottingRequest/plottingRequest_SystemsReport.xlsx',...
  PLOT_CONFIG_FILES =  '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/plottingRequest/plottingRequest_E06DAR_PR.xlsx'
      % };
SEASON_FILE       = '/Users/lholliday/SimEnv/trackDB/seasonOverview.xlsx';

PPTX_TEMPLATE     = '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/plot/templates/SuperCars_PPT.pptx';
OUTPUT_DIR        = '/Users/lholliday/SimEnv/dataAcquisition/Motec_MP/plot/output';
...
%% =========================================================
%  SECTION 4b: PRE-CONCAT TEAMDATA (optional)
%  Run smp_concat_teamdata to produce clean HOL files first,
%  then point compile at the HOL output directory instead of
%  the raw _TeamData directory.
% =========================================================
RUN_TEAMDATA_CONCAT = true;   % set true on first run or when new files arrive

if RUN_TEAMDATA_CONCAT
    concat_cfg.root_folder       = TOP_LEVEL_DIR;
    concat_cfg.td_input_dir      = TOP_LEVEL_DIR;
    concat_cfg.td_hol_output_dir = fullfile(TOP_LEVEL_DIR, '_HOL');
    concat_cfg.hol_venue         = EVENT_NAME;
    concat_cfg.hol_event         = sprintf('%s%s', EVENT, TRACK);
    concat_cfg.event_alias_file  = EVENT_ALIAS_FILE;
    concat_cfg.driver_alias_file = DRIVER_ALIAS_FILE;
    concat_cfg.session_filter    = SESSION_FILTER;
    concat_cfg.unique_fp         = true;
    concat_cfg.show_report       = false;
    concat_cfg.overwrite         = false;
    concat_cfg.driver_filter     = {};
    concat_cfg.team_filter       = TEAM_FILTER;
...
if RUN_RECOMPUTE_VCH
    ...
    switch lower(RECOMPUTE_MODE)
        ...
        % ----------------------------------------------------------
            ...
            warning('Needs a refactor | Script Recalculates the lap boundaries')
            cache = smp_recompute_vch(COMPILE_DIR_Sesh, season, driver_map, alias, ...
            ...
            for w = 1:N_WORKERS
                ...
                for k = 1:numel(keys)
                    ...
                    if ~isfield(cache.stats, gk)
                        cache.stats.(gk) = struct();
                    end
                    vch_keys = fieldnames(partial.vch_stats.(gk));
                    for v = 1:numel(vch_keys)
                        cache.stats.(gk).(vch_keys{v}) = partial.vch_stats.(gk).(vch_keys{v});
                    end
                    % Backfill GroupKey in manifest rows that are still empty
                    empty_gk = strcmp(cache.manifest.GroupKey, '');
                    if any(empty_gk) && isfield(partial, 'group_files') && isfield(partial.group_files, gk)
                        grp_files_cell = cellstr(partial.group_files.(gk));
                        for fi = find(empty_gk)'
                            if ismember(cache.manifest.Path{fi}, grp_files_cell)
                                cache.manifest.GroupKey{fi} = gk;
                            end
                        end
                    end
...
if CREATE_PITSTOP_REPORT
    ...
    switch lower(MODE)
        ...
        % ------------------------------------------------------
            ...
            [pitData, pit_summary] = smp_stops_to_pitdata(stops, SMP_filtered, driver_map);
            figs    = plotPitStops(pitData, 'Cfg', cfg, 'DriverMap', driver_map);
            if ~isempty(pit_summary)
                pit_sess_str = strjoin(SESSION_FILTER, '_');
                pit_csv_name = sprintf('pitstops_%s%s_%s.csv', EVENT, TRACK, pit_sess_str);
                pit_csv_path = fullfile(OUTPUT_DIR, pit_csv_name);
                writetable(pit_summary, pit_csv_path);
                fprintf('  Pit summary written to: %s\n', pit_csv_path);
            end
