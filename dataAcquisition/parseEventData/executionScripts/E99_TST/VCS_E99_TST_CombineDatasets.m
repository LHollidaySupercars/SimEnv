...
PHASE1_MODE        = 'serial';   % 'serial' | 'parallel'
N_WORKERS          = 4;       % 'auto' = one worker per driver, or integer e.g. 8
...
PHASE2_MODE        = 'serial';   % 'serial' | 'parallel'
N_WORKERS_ECU      = 8;       % 'auto' = one worker per driver, or integer e.g. 8
...
PHASE4_MODE        = 'serial';   % 'serial' | 'parallel'
N_WORKERS_PAIR     = 4;       % number of workers for Phase 4 pairing
...
PHASE6_MODE            = 'serial';   % 'serial' | 'parallel'
N_WORKERS_AUG          = 4;            % number of workers for Phase 6 augmentation
...
% =========================================================================
cfg.root_folder  = 'E:\2026\E99_TST';   % event root — all paths derived from here
cfg.hol_venue    = 'Hidden Valley';
cfg.hol_event    = 'E99_TST';
cfg.event_date   = '2026-06-19';         % used by Phase 1 (TeamData date filter) and Phase 4b (L180 sort)
...
RUN_SIMULINK_L180      = true;   % Phase 6b: run Simulink model on L180 files, append outputs to struct

cfg.simulink_model_name = 'V8_mdl';   % <-- set to your .slx model name (no extension)
cfg.simulink_stop_time = [];   % [] = auto from L180 data length, or e.g. 120

cfg.simulink_params = struct( ...
    'Vehicle_Mass',   1350, ...      % -> model workspace variable
    'Tyre_Mu',        1.05, ...
    'block_params',   {{                                   ...
        {[cfg.simulink_model_name '/Gain'], 'Gain', '0.9'} ...
    }} ...
);
% =========================================================================
...
if RUN_AUGMENT_CHANNELS
    fprintf('\n%s\n', repmat('=', 1, 60));
    fprintf('PHASE 6 — Augment COM files  [%s]\n', cfg.session);
    fprintf('%s\n', repmat('=', 1, 60));

    if strcmp(PHASE6_MODE, 'serial')
        augment_com_files_serial(cfg.com_dir, CHANNELS_FILE, cfg);
    else
        launch_parallel_workers_augment(cfg.com_dir, CHANNELS_FILE, cfg, ...
            N_WORKERS_AUG, KEEP_WORKERS_AUG_OPEN, TMP_DIR_AUG);
    end
end
...
% =========================================================================
%%  PHASE 7 — L180 SIMULINK AUGMENTATION
% =========================================================================
%%  PHASE 7 — L180 SIMULINK AUGMENTATION
% =========================================================================
if RUN_SIMULINK_L180
    fprintf('\n%s\n', repmat('=', 1, 60));
    fprintf('PHASE 7 — L180 Simulink augmentation  [%s]\n', cfg.session);
    fprintf('%s\n', repmat('=', 1, 60));

    cfg.simulink_params = struct( ...
        'transmissionRatio', 3.273, ...
        'aero',       struct('rho', 1.225), ...
        'RAD2DEG_V',  180/pi, ...
        'block_params', {{ ...
            {[cfg.simulink_model_name '/Gain'], 'Gain', '0.9'} ...
        }} ...
    ...
    all_paths_sl = recursive_find_ld(cfg.com_dir);
    l180_paths   = all_paths_sl(contains(lower(all_paths_sl), 'l180'));

    if isempty(l180_paths)
        fprintf('  [WARN] No L180 files found in %s\n', cfg.com_dir);
    else
        load_system(cfg.simulink_model_name);

        % ---- L180 signal bus: define once, reused every file ----
        cfg.simulink_channels = {'Ground_Speed', 'Steering_Angle', ...
            'Wheel_Speed_Yaw_Rate', 'Acceleration_Y_Filt'};
        cfg.simulink_bus_name = 'l180_bus';
        smp_ensure_bus_object(cfg.simulink_channels, cfg.simulink_bus_name);

        % ---- Vehicle constants bus: define once ----
        % *** CONFIRM: what variable does GEN3_KinematicParameters actually
        % create? Run it once manually and check `whos` — if it's not a
        % plain scalar struct named `vehicle`, update the line below. ***
        GEN3_KinematicParameters
           % <-- FIX: replace with real var name/conversion
        cfg.simulink_vehicle_bus_name = 'vehicleBus';
        smp_ensure_bus_from_struct(vehicle, cfg.simulink_vehicle_bus_name);
        assignin('base', 'vehicle', vehicle);

        cfg.simulink_init_scripts = { ...
            fullfile(cfg.root_folder, 'sim_init', 'load_transmission_ratio.m'), ...
            fullfile(cfg.root_folder, 'sim_init', 'load_aero_constants.m'), ...
        };

        n_sl_ok = 0; n_sl_fail = 0;
        for fi = 1:numel(l180_paths)
            sl_path = l180_paths{fi};
            [~, sl_fname] = fileparts(sl_path);
            fprintf('  [%d/%d] %s\n', fi, numel(l180_paths), sl_fname);
            try
                sl_data      = motec_ld_reader(sl_path, {});
                sl_data.info = motec_ld_info(sl_path, false);
                sl_data = smp_run_l180_simulink(sl_data, cfg);
                n_sl_ok = n_sl_ok + 1;
            catch ME
                disp(getReport(ME, 'extended'));
                n_sl_fail = n_sl_fail + 1;
            end
            clear sl_data;
        end
        fprintf('  Phase 7 complete — OK: %d  Failed: %d\n', n_sl_ok, n_sl_fail);
    end
end


%%
evalin('base', 'clear l180_bus vehicleBus');
delete(which('l180_bus.m'));
delete(which('vehicleBus.m'));

% Step 1 — create l180_bus, check immediately
smp_ensure_bus_object(cfg.simulink_channels, 'l180_bus');
class(evalin('base', 'l180_bus'))   % <-- check here

% Step 2 — create vehicleBus, check immediately
smp_ensure_bus_from_struct(vehicle, 'vehicleBus');
class(evalin('base', 'vehicleBus'))   % <-- check here
%%
...
function augment_com_files_serial(com_dir, channels_file, cfg)
...
%   Falls back to driver-alias lookup for manufacturer when the .ld
%   header doesn't carry one.
    ...
    % ---- Load driver alias map for manufacturer fallback ----
    driver_map = [];
    if isfield(cfg, 'driver_alias_file') && isfile(cfg.driver_alias_file)
        try
            driver_map = smp_driver_alias_load(cfg.driver_alias_file);
        catch
            fprintf('  [WARN] Could not load driver alias — manufacturer fallback disabled.\n');
        end
    end
    ...
    for fi = 1:numel(all_paths)
        ...
        try
            ...
            if (~isfield(aug_data.info, 'manufacturer') || isempty(aug_data.info.manufacturer)) && ...
               ...
               ~isempty(driver_map)
                mfr = local_resolve_manufacturer(aug_data.info.driver, driver_map);
                if ~isempty(mfr)
...
function mfr = local_resolve_manufacturer(driver_name, driver_map)
% LOCAL_RESOLVE_MANUFACTURER  Resolve manufacturer (MAN) for a driver from
%   the driver alias map. Trimmed, non-interactive copy of the lookup logic
%   in smp_compile_event.m (direct key -> strip-normalised key -> alias
%   search). Returns '' silently on no match — no keyboard pause, safe for
%   batch/parallel Phase 6 runs.
    mfr = '';
    ...
    if ~isempty(entry_found) && isfield(entry_found, 'manufacturer') && ~isempty(entry_found.manufacturer)
        mfr = entry_found.manufacturer;
    end
end


function launch_parallel_workers_augment(com_dir, channels_file, cfg, N_WORKERS, KEEP_WORKERS_OPEN, TMP_DIR)
...
%   files across workers. Each worker calls smp_augment_com_worker.
    ...
    aug_worker_cfg.channels_file = channels_file; %#ok<NASGU>
...
ALIAS_DIR = 'C:\SimEnv\dataAcquisition\Motec_MP\alias';

% ---- Alias files ----
cfg.event_alias_file  = fullfile(ALIAS_DIR, cfg.hol_event, 'eventAlias.xlsx');
...
cfg.td_hol_output_dir = fullfile(cfg.root_folder, '_HOL');
...
function data = smp_run_l180_simulink(data, cfg)
% SMP_RUN_L180_SIMULINK  Configure and run a Simulink model against one
% L180 data struct, then append its logged outputs back into the struct.

    model_name = cfg.simulink_model_name;
    if isempty(model_name)
        error('smp_run_l180_simulink:noModel', 'cfg.simulink_model_name is not set.');
    end

    busName = cfg.simulink_bus_name;
    load_system(model_name);

    in = Simulink.SimulationInput(model_name);

    % ---- Init scripts (transmissionRatio, aero.rho, etc.) ----
    if isfield(cfg, 'simulink_init_scripts') && ~isempty(cfg.simulink_init_scripts)
        in = smp_run_init_scripts(in, cfg.simulink_init_scripts);
    end

    % ---- Bus-fed L180 channels (per-file data) ----
    ws_input = smp_build_dataset_input(data, cfg.simulink_channels);
    assignin('base', 'l180_input_data', ws_input);
    in = in.setVariable('l180_input_data', ws_input);
    in = smp_configure_bus_input(in, model_name, [model_name '/From Workspace'], busName);
    busObj = evalin('base', busName);
    in = in.setVariable(busName, busObj);

    % ---- Vehicle constants bus (static, same every file) ----
    if isfield(cfg, 'simulink_vehicle_bus_name') && ~isempty(cfg.simulink_vehicle_bus_name)
        vehicleBusObj = evalin('base', cfg.simulink_vehicle_bus_name);
        in = in.setVariable('vehicle', evalin('base', 'vehicle'));
        in = in.setVariable(cfg.simulink_vehicle_bus_name, vehicleBusObj);
    end

    % ---- Duration ----
    stop_time = resolve_stop_time(data, cfg);
    in = in.setModelParameter('StopTime', num2str(stop_time));
    fprintf('    StopTime = %.3f s\n', stop_time);

    % ---- Tunable params / gains / constants ----
    if isfield(cfg, 'simulink_params') && ~isempty(cfg.simulink_params)
        in = apply_simulink_params(in, cfg.simulink_params);
    end
    ...
    data = smp_simout_to_channels(data, simOut);
    fprintf('    Simulink run complete.\n');
end


function stop_time = resolve_stop_time(data, cfg)
    if isfield(cfg, 'simulink_stop_time') && ~isempty(cfg.simulink_stop_time)
        stop_time = cfg.simulink_stop_time;
        return;
    end
    ref = data.Engine_Speed;
    if isstruct(ref) && isfield(ref, 'time') && ~isempty(ref.time)
        stop_time = ref.time(end) - ref.time(1);
    else
        error('smp_run_l180_simulink:noStopTime', ...
            'Could not derive StopTime from data - set cfg.simulink_stop_time explicitly.');
    end
end


function in = smp_run_init_scripts(in, script_paths)
    for si = 1:numel(script_paths)
        script_path = script_paths{si};
        if ~isfile(script_path)
            fprintf('    [WARN] Init script not found: %s - skipping\n', script_path);
            continue;
        end
        vars_before = who;
        run(script_path);
        vars_after  = who;
        new_vars = setdiff(vars_after, [vars_before; {'vars_before'; 'si'; 'script_paths'; 'script_path'; 'in'; 'new_vars'}]);
        for vi = 1:numel(new_vars)
            vname = new_vars{vi};
            in = in.setVariable(vname, eval(vname));
            fprintf('    [init] %s <- %s\n', vname, script_path);
        end
    end
end
...
function in = apply_simulink_params(in, params)
    names = fieldnames(params);
    for i = 1:numel(names)
        name = names{i};
        if strcmp(name, 'block_params') && iscell(params.(name))
            bp = params.(name);
            for bi = 1:numel(bp)
                block_path = bp{bi}{1};
                h = getSimulinkBlockHandle(block_path, false);
                if isequal(h, -1)
                    fprintf('    [WARN] Block not found: %s - skipping\n', block_path);
                    continue;
                end
                in = in.setBlockParameter(block_path, bp{bi}{2}, bp{bi}{3});
                fprintf('    [block param] %s.%s = %s\n', block_path, bp{bi}{2}, num2str(bp{bi}{3}));
            end
            continue;
        end
        in = in.setVariable(name, params.(name));
        fprintf('    [var] %s = %s\n', name, safe_display(params.(name)));
    end
end


function s = safe_display(val)
    if isnumeric(val) || ischar(val) || isstring(val)
        s = mat2str(val);
    elseif isstruct(val)
        s = jsonencode(val);
    else
...
function ws = smp_build_dataset_input(data, channel_list)
    ws = struct();
    for i = 1:numel(channel_list)
        ch_name = channel_list{i};
        if ~isfield(data, ch_name)
            fprintf('    [WARN] Channel "%s" not found in data - skipping.\n', ch_name);
            continue;
        end
        ch = data.(ch_name);
        ws.(ch_name) = timeseries(ch.data(:), ch.time(:), 'Name', ch_name);
    end
end


function in = smp_configure_bus_input(in, model_name, block_path, busName)
    set_param(block_path, 'OutDataTypeStr', ['Bus: ' busName]);
end


function busName = smp_ensure_bus_object(channel_list, busName)
    if nargin < 2 || isempty(busName)
        busName = 'l180_bus';
    end

    needs_rebuild = true;
    if evalin('base', sprintf('exist(''%s'', ''var'')', busName))
        existing = evalin('base', busName);
        if isa(existing, 'Simulink.Bus')
            existing_names = {existing.Elements.Name};
            if isequal(sort(existing_names), sort(channel_list))
                fprintf('  [bus] "%s" already matches channel list - reusing.\n', busName);
                needs_rebuild = false;
            else
                fprintf('  [bus] "%s" exists but shape changed - rebuilding.\n', busName);
            end
        end
    end

    if needs_rebuild
        evalin('base', sprintf('clear %s', busName));
        sample_ws = struct();
        for i = 1:numel(channel_list)
            sample_ws.(channel_list{i}) = timeseries(0, 0);
        end
        bus_file = which([busName '.m']);
        if ~isempty(bus_file) && isfile(bus_file)
            delete(bus_file);
        end
        Simulink.Bus.createObject(sample_ws, busName);
        fprintf('  [bus] "%s" created for channels: %s\n', busName, strjoin(channel_list, ', '));
    end

    evalin('base', busName);
    fprintf('  [bus] "%s" loaded into base workspace.\n', busName);
end


function busName = smp_ensure_bus_from_struct(sample_struct, busName)
    if ~isstruct(sample_struct) || numel(sample_struct) ~= 1
        error('smp_ensure_bus_from_struct:badInput', ...
            'Expected a scalar struct, got a %s. Convert your vehicle params to a plain struct first.', ...
            class(sample_struct));
    end
    ...
    if evalin('base', sprintf('exist(''%s'', ''var'')', busName))
        ...
        if isa(existing, 'Simulink.Bus')
            existing_names = {existing.Elements.Name};
            if isequal(sort(existing_names), sort(field_names))
                fprintf('  [bus] "%s" already matches struct - reusing.\n', busName);
                needs_rebuild = false;
            else
                fprintf('  [bus] "%s" exists but shape changed - rebuilding.\n', busName);
            end
    ...
    if needs_rebuild
        ...
        Simulink.Bus.createObject(sample_struct, busName);
        fprintf('  [bus] "%s" created for fields: %s\n', busName, strjoin(field_names, ', '));
    end
...
function name = get_block_name(path)
    parts = strsplit(path, '/');
    name = parts{end};
end


function data = smp_simout_to_channels(data, simOut)
    if ~isfield(simOut, 'logsout') || isempty(simOut.logsout)
        fprintf('    [WARN] No logsout data found in simOut - nothing appended.\n');
        return;
    end
    logs = simOut.logsout;
    n_sig = logs.numElements;
    for i = 1:n_sig
        elem = logs.getElement(i);
        sig_name  = matlab.lang.makeValidName(elem.Name);
        ts        = elem.Values;
        sig_time  = ts.Time(:);
        sig_data  = ts.Data(:);
        if isempty(sig_time) || numel(sig_time) < 2
            fprintf('    [WARN] Skipping "%s" - insufficient samples.\n', sig_name);
            continue;
        end
        dt = median(diff(sig_time));
        sample_rate = round(1 / dt);
        ch.data        = sig_data;
        ch.time        = sig_time;
        ch.units       = '';
        ch.sample_rate = sample_rate;
        ch.raw_name    = elem.Name;
        ch.write_to_ld = false;
        data.(sig_name) = ch;
        fprintf('    [+] %s  (%d samples @ %d Hz)\n', sig_name, numel(sig_data), sample_rate);
    end
end