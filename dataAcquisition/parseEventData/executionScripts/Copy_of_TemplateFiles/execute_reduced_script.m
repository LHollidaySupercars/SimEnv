%% =========================================================
%  EXECUTE_REDUCED_SCRIPT
%  =========================================================
%  Compile, VCH recompute, write VCH .ld files, plots (no PPTX, no upload).
%
%  WORKFLOW:
%    Step 1  — Edit CONFIG sections (1–3)
%    Step 2  — Run Section 5  to compile new .ld files
%           OR Section 5b to load cache only (re-plot / re-write, no compile)
%    Step 3  — Section 5c: VCH recompute  (set RUN_RECOMPUTE_VCH = true)
%    Step 4  — Section 5d: VCH debug plot (set VCH_DEBUG_PLOT  = true)
%    Step 5  — Section 5e: Write VCH channels → <name>_VCH.ld files
%    Step 6  — Section 8:  Plots (figures only — no PPTX, no upload)
%
...
%
%  VCH WRITE:
%    Each source .ld in the manifest gets a matching <name>_VCH.ld
%    alongside it containing all smp_custom_channels + smp_gated_channels
%    outputs.  Set WRITE_VCH_LD = false to skip.
%    Set OVERWRITE = true to replace existing _VCH files and cache.
%
%  DUPLICATE DETECTION:
%    uniqueFingerprint is always enabled — GPS probe matching with
%    start+end speed fingerprint fallback (see concat_sessions.m).
% =========================================================
...
TOP_LEVEL_DIR     = 'E:\2026\T01_QLR\COM - copy';
...
OVERWRITE = true;             % true = replace existing _VCH.ld and cache .mat files
...
% ---- VCH .ld write options ----
WRITE_VCH_LD  = true;   % true = write VCH channels back into .ld files
VCH_LD_SUFFIX = '_VCH'; % appended before .ld extension
...
compile_opts.concat_csv_dir    = OUTPUT_DIR;  % '' = skip CSV
...
compile_opts.uniqueFingerprint = true;        % always-on duplicate detection
...
% ---- Purge existing cache if OVERWRITE = true ----
if OVERWRITE
    existing_cache = dir(fullfile(TOP_LEVEL_DIR, 'smp_cache*.mat'));
    if ~isempty(existing_cache)
        fprintf('OVERWRITE = true — deleting %d cache file(s)...\n', numel(existing_cache));
        for ci = 1:numel(existing_cache)
            delete(fullfile(existing_cache(ci).folder, existing_cache(ci).name));
            fprintf('  Deleted: %s\n', existing_cache(ci).name);
        end
    end
end
...
%% =========================================================
%  SECTION 5b: LOAD ONLY  (re-plot/re-write without recompiling)
...
%% =========================================================
%  SECTION 5e: WRITE VCH CHANNELS TO .LD FILES
%  Re-reads each source .ld from the manifest, reruns
%  smp_custom_channels + smp_gated_channels, and writes all
%  produced channels into a new <name>_VCH.ld alongside the
%  original.  One output file per source .ld.
%
%  Gate: WRITE_VCH_LD (Section 3)
%  Skip: OVERWRITE_VCH_LD = false skips existing output files.
% =========================================================

if WRITE_VCH_LD
    fprintf('\n=== VCH → .ld Write ===\n\n');

    % ---- Gather unique source files (LoadOK only) ----
    mf_ok     = cache.manifest(cache.manifest.LoadOK, :);
    src_files = unique(mf_ok.Path, 'stable');
    n_files   = numel(src_files);
    ...
    for fi = 1:n_files
        ...
        [d, base, ext] = fileparts(src_file);
        cbm_dir  = fullfile(d, 'CBM');
        if ~exist(cbm_dir, 'dir'), mkdir(cbm_dir); end
        out_file = fullfile(cbm_dir, [base VCH_LD_SUFFIX ext]);

        fprintf('[%d/%d] %s\n', fi, n_files, base);

        % ---- Skip checks ----
        if ~OVERWRITE && exist(out_file, 'file')
            fprintf('  SKIP — output already exists\n');
            vch_n_skip = vch_n_skip + 1;
            continue;
        end
        ...
        try
            % ---- Read .ld ----
            fprintf('  Reading...\n');
            vch_data = motec_ld_reader(src_file, {});

            % ---- Resolve manufacturer (motec_ld_info → driver_map fallback) ----
            vch_mfr = '';
            try
                vch_info = motec_ld_info(src_file, false);
                if isfield(vch_info, 'manufacturer'), vch_mfr = vch_info.manufacturer; end
            catch
                vch_info = struct();
            end

            if isempty(vch_mfr) && ~isempty(driver_map)
                vch_drv = '';
                if isfield(vch_info, 'driver'), vch_drv = vch_info.driver; end
                if ~isempty(vch_drv)
                    vch_key   = regexprep(lower(strtrim(vch_drv)), '[^a-z0-9]', '');
                    dm_names  = fieldnames(driver_map);
                    vch_entry = [];
                    % 1. Direct key
                    if isfield(driver_map, vch_drv), vch_entry = driver_map.(vch_drv); end
                    % 2. Normalised key
                    if isempty(vch_entry)
                        for dmi = 1:numel(dm_names)
                            if strcmp(vch_key, regexprep(lower(dm_names{dmi}), '[^a-z0-9]', ''))
                                vch_entry = driver_map.(dm_names{dmi}); break;
                            end
                        end
                    end
                    % 3. Alias search
                    if isempty(vch_entry)
                        for dmi = 1:numel(dm_names)
                            ...
                            if ~isfield(e, 'aliases'), continue; end
                            for ai = 1:numel(e.aliases)
                                if strcmp(vch_key, regexprep(lower(e.aliases{ai}), '[^a-z0-9]', ''))
                                    vch_entry = e; break;
                                end
                            end
                            if ~isempty(vch_entry), break; end
                        end
                    end
                    if ~isempty(vch_entry) && isfield(vch_entry, 'manufacturer')
                        vch_mfr = vch_entry.manufacturer;
                    end
                end
            end

            % ---- Compute VCH channels ----
            fprintf('  Computing VCH channels...\n');
            vch_data = smp_custom_channels(vch_data, 'manufacturer', vch_mfr);
            [vch_data, gated_names] = smp_gated_channels(vch_data, T_gated);

            % ---- Collect channels to write ----
            ch_list  = {};
            all_flds = fieldnames(vch_data);

            % Custom channels: all fields with write_to_ld = true
            for fj = 1:numel(all_flds)
                f   = all_flds{fj};
                fch = vch_data.(f);
                if ~isstruct(fch),                              continue; end
                if ~isfield(fch, 'write_to_ld') || ~fch.write_to_ld, continue; end

                ld_ch             = struct();
                ld_ch.name        = fch.raw_name;
                ld_ch.units       = fch.units;
                ld_ch.sample_rate = fch.sample_rate;
                ld_ch.value       = fch.data(:);
                ld_ch.dec_places  = fch.dec_places;
                ld_ch.mul         = 1;
                ld_ch.scale       = 1;
                ld_ch.offset      = 0;
            ...
            % Gated channels: returned by name from smp_gated_channels
            for gj = 1:numel(gated_names)
                gn  = gated_names{gj};
                if ~isfield(vch_data, gn), continue; end
                gch = vch_data.(gn);
                if ~isstruct(gch), continue; end

                ld_ch             = struct();
                ld_ch.name        = gn;
                ld_ch.units       = gch.units;   % '' clears donor units; [] inherits
                ld_ch.sample_rate = gch.sample_rate;
                ld_ch.value       = gch.data(:);
                if isfield(gch, 'dec_places')
                    ld_ch.dec_places = gch.dec_places;
                else
                    ld_ch.dec_places = 2;
                end
                ld_ch.mul    = 1;
            ...
            if isempty(ch_list)
                fprintf('  No VCH channels to write — skipping\n');
                vch_n_skip = vch_n_skip + 1;
                continue;
            end

            % ---- Write ----
            fprintf('  Writing %d channel(s) → %s%s%s\n', ...
                numel(ch_list), base, VCH_LD_SUFFIX, ext);
            ld_add_channel(src_file, out_file, ch_list);
            fprintf('  OK\n');
            vch_n_ok = vch_n_ok + 1;

        catch ME_vch
            fprintf('  ERROR: %s\n', ME_vch.message);
            vch_n_fail = vch_n_fail + 1;
        end
    end

    fprintf('\n=== VCH Write — OK=%d  Skipped=%d  Failed=%d ===\n\n', ...
        vch_n_ok, vch_n_skip, vch_n_fail);
end
...
if CREATE_PITSTOP_REPORT
    ...
    switch lower(MODE)
        ...
        % ------------------------------------------------------
            ...
            n_pit_workers = min(N_WORKERS, n_teams);
...
%% =========================================================
%  SECTION 8: PLOTS (figures only — no PPTX)
% =========================================================
if PLOTTING
