...
SESSION   = 'Q23';   % session to process (used in all phases)
...
% ---- Hard reset: clear any stale bus/model state from previous runs ----
clear mex
bdclose('all')
evalin('base', 'clearvars -regexp ^VehicleBus$|^AugInputBus$|^slBus');

% ---- Load vehicle kinematic parameters ----
GEN3_KinematicParameters_simulink;   % populates 'vehicle' in the workspace

vFields = fieldnames(vehicle);
fprintf('vehicle struct fields (%d): %s\n', numel(vFields), strjoin(vFields, ', '));

% ---- Config ----
chanList = {'Steering_Angle', 'Ground_Speed', 'Acceleration_Y_Filt', ...
            'Wheel_Speed_Front_Left', 'Wheel_Speed_Front_Right','L180.L180_Acceleration Y'};

com_path{1}  = 'F:\2026\E08_PER\COM\R25\01_T8R\FEE_2026_R25_combined_l180.ld';
% com_path{2}  = 'E:\2026\E08_PER\_HOL\teamData\Q23\03_WTG\MOS_2026_Q23_combined_l180.ld';
% com_path{3}  = 'E:\2026\E08_PER\_HOL\teamData\Q23\10_T18\DEP_2026_Q23_combined_l180.ld';
% com_path{4}  = 'E:\2026\E08_PER\_HOL\teamData\Q23\10_T18\REY_2026_Q23_combined_l180.ld';

chanList = cellfun(@sanitizeChannelName, chanList, 'UniformOutput', false);
modelName = 'V8_mdl_KF_imp';

params.rTyreFront = 320;
params.rTyreRear  = 330;
params.mass       = 1350;
params.RAD2DEG_V  = 180/pi;
params.DEG2RAD_V  = pi/180;
...
% ---- Build vehicle bus: clear stale .m file on path + rebuild fresh ----
allCopies = which('VehicleBus.m', '-all');
if ~isempty(allCopies)
    for k = 1:numel(allCopies)
        fprintf('Deleting stale bus file: %s\n', allCopies{k});
        delete(allCopies{k});
    end
    rehash path
end

busInfo = Simulink.Bus.createObject(params.vehicle);
assignin('base', 'VehicleBus', evalin('base', busInfo.busName));
evalin('base', ['clear ' busInfo.busName]);

b = evalin('base', 'VehicleBus');
busElemNames = {b.Elements.Name}';
fprintf('VehicleBus built with %d top-level element(s): %s\n', ...
    numel(busElemNames), strjoin(busElemNames, ', '));

missing = setdiff(vFields, busElemNames);
if ~isempty(missing)
    error('combine_datasets:busMismatch', ...
        'VehicleBus is missing field(s) present in vehicle struct: %s', strjoin(missing, ', '));
end

%% ---- Load .ld file(s) and run sim ----
...
% Variable-step solver (fixed-step discrete broke on non-discrete blocks
% elsewhere in the model earlier — keep variable-step; discrete
% subsystems inside still run at their own explicit sample time).
set_param(modelName, 'SolverType', 'Variable-step');
set_param(modelName, 'Solver', 'ode45');

% run_sim_augment computes Ts_common from the REAL resampled data inside
% build_sim_input_dataset and sets MaxStep/InitialStep itself, so no
% separate/duplicate Ts_common calculation needed here anymore.
...
function simOut = run_sim_augment(modelName, aug_data, chanList, params)
    [ds, Ts_common] = build_sim_input_dataset(aug_data, chanList);

    fprintf('Solver MaxStep set to %.5f s\n', Ts_common);
    set_param(modelName, 'MaxStep', num2str(Ts_common));
    set_param(modelName, 'InitialStep', num2str(Ts_common));

    params.Ts = Ts_common;   % ensures bicycleModelStateSpaceMatrix uses the
                              % SAME Ts as the actual resampled data/solver
...
function [aug_data, newChannels] = extract_new_sim_channels(aug_data, simOut, preNames)
    ...
    outputFields = {'bicycleModel', 'bicycleModel1'};
    for oi = 1:numel(outputFields)
        ...
        for si = 1:numel(s.signals)
            ...
            if isfield(aug_data, name), continue; end
...
function [ds, Ts_common] = build_sim_input_dataset(chan, chanList)
% BUILD_SIM_INPUT_DATASET  Resamples all channels in chanList to a common
% rate (the fastest native sample_rate among them) before building the
% Simulink Dataset input, so every channel is genuinely fresh at every
% simulation timestep rather than being implicitly held/interpolated
% per-channel by Simulink's own From Workspace mechanism.

    % ---- Determine the common target rate: fastest channel present ----
    rates = zeros(numel(chanList), 1);
    for i = 1:numel(chanList)
        name = chanList{i};
        if isfield(chan.(name), 'sample_rate') && ~isempty(chan.(name).sample_rate)
            rates(i) = chan.(name).sample_rate;
        else
            t = chan.(name).time(:);
            rates(i) = 1 / median(diff(t));
        end
    end

    fastestRate = max(rates);
    Ts_common   = 1 / fastestRate;

    fprintf('Channel sample rates (Hz): %s\n', mat2str(rates', 4));
    fprintf('Using common rate: %.4f Hz (Ts = %.5f s)\n', fastestRate, Ts_common);

    % ---- Build a common time vector spanning the overlapping range ----
    tStart = -Inf;
    tEnd   =  Inf;
    for i = 1:numel(chanList)
        name = chanList{i};
    ...
    t_common = (tStart : Ts_common : tEnd)';

    % ---- Resample every channel onto t_common, build struct-of-timeseries ----
    busData = struct();
    for i = 1:numel(chanList)
        name = chanList{i};
        t = chan.(name).time(:);
        ...
        v_resampled = interp1(t, v, t_common, 'linear', 'extrap');

        ts = timeseries(v_resampled, t_common, 'Name', name);
    ...
    ds = ds.addElement(busData, 'Road Input');
end
...
function cleanName = sanitizeChannelName(rawName)
    cleanName = regexprep(rawName, '[^a-zA-Z0-9_]', '_');
    if ~isletter(cleanName(1))
        cleanName = ['ch_' cleanName];
    end
end