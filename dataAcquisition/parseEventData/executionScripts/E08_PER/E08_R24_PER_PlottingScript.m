...
cfg.root_folder      = 'F:\2026\E08_PER\_HOL\teamData'
...
cfg.date_from        = datetime(2026, 8, 1);   % auto-filled from sessionDate.xlsx (this session's date)
...
% cfg.channel_mode = 'all';   % 'restricted' = plot-driven list (fast)
                                    % 'all'        = every channel, full math (slow, debug/tyre-change use)
...
switch cfg.channel_mode
    case 'restricted'
        ...
        INFRA_REQUIRED = {'Lap_Number', 'Lap_Time', 'Beacon', ...
                           ...
                           'TPM1S FL WS ID', 'TPM1S FR WS ID', ...
                           'TPM1S RL WS ID', 'TPM1S RR WS ID'};
        ...
        cfg.load_all_channels = false;


    case 'all'
        channels = channels;          % full list from smp_channel_config_load (PHASE 1)
        cfg.load_all_channels = true; % skip channel filtering at .ld read
        compile_opts.channel_ops_map = '';
    otherwise
        error('Unknown cfg.channel_mode "%s" — use ''restricted'' or ''all''.', cfg.channel_mode);
end
...
if PHASE_5_TYRE_CHANGE
    ...
    saveLocation = fullfile(pwd, 'dataAcquisition\parseEventData\pitStop', ...
                             join([cfg.event, '_', cfg.track], '_'));
    if ~isfolder(saveLocation)
        mkdir(saveLocation)
    end
    writetable(pit_summary, fullfile(saveLocation, '26VCS_PitStop_PER_E08_R24.xlsx'))
end
