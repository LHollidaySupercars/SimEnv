...
SESSION   = 'Q21';   % session to process (used in all phases)
...
% =========================================================================
cfg.root_folder  = 'E:\2026\E07_TSV';   % event root — all paths derived from here
cfg.hol_venue    = 'Townsville';
cfg.hol_event    = 'E07_TSV';
cfg.event_date   = '2026-07-11';         % used by Phase 1 (TeamData date filter) and Phase 4b (L180 sort)
...
Car     = [31, 2, 17, 1, 31, 6, 17, 19, 1, 26, 3, 38, 55, 7, 6, 2, 17, 191,1, 20, 99, 19, 18];
FrontRH = [97, 78, 91,   82, 97, 86, 87, 95, 76, 100, 107, 90, 88, 104, 84, 77, 90, 99, 81, 92, 93, 95, 95];
RearRH  = [142, 158, 135, 136, 141, 132, 135, 144, 141, 145, 136, 135, 131, 141, 133, 139, 134, 147,135, 139, 141, 143, 144];
Session = {'Q20','Q20','Q20','R20','R20','R20','R20','R20','Q21','Q21','Q21','Q21','Q21','Q21','Q21','Q21','Q21','Q21','R21','R21','R21','R21','R21'};
length(Car)
length(FrontRH)
length(RearRH)

Driver = cell(1, length(Car));
MAN    = cell(1, length(Car));
for i = 1:length(Car)
    Driver{i} = resolve_driver_tla(Car(i), driver_map);
    MAN{i}    = local_resolve_manufacturer_by_tla(Driver{i}, driver_map);
end

cfg.PatchRH = table(Car', Driver', MAN', FrontRH', RearRH', Session', ...
...
% ---- Alias files ----
cfg.event_alias_file  = fullfile('C:\SimEnv\dataAcquisition\parseEventData\executionScripts', cfg.hol_event, 'eventAlias.xlsx');
