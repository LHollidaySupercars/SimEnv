%% =========================================================
%  EXECUTE_MAIN_REPORT  (post-VCS pipeline)
%
%  Workflow dependency:
%    Run VCS_E07_TSV_CombineDatasets.m (or equivalent) BEFORE this
%    script.  That pipeline handles all data combination (TeamData concat,
%    ECU concat, split, pair, L180, augment) and writes COM files.
%    This script compiles statistics from those COM files, detects tyre
%    changes, generates plots, uploads, and saves the cache.
% =========================================================
clear; clc; close all;
t_script = tic;
...
TOP_LEVEL_DIR     = 'E:\2026\E07_TSV';          % event root (same as VCS script root_folder)
...
% COM subfolder written by VCS pipeline; adjust if VCS uses a different name
COM_SUBDIR            = 'COM';        % relative to TOP_LEVEL_DIR
...
worker_opts.n_workers         = 4;
worker_opts.tmp_dir           = fullfile(tempdir, 'smp_parallel');
worker_opts.poll_interval_s   = 3;
worker_opts.timeout_s         = 3600;
worker_opts.keep_workers_open = false;   % false = cmd /c (auto-close on success)
                                          % true  = cmd /k (leave window open — debugging)
...
compile_opts.load_all_channels = true;      % true = load full file (recommended for COM files)
...
[channels, channel_rules, alias, driver_map, cfg, T_gated, season] = ...
    load_event_config(CHANNELS_FILE, EVENT_ALIAS_FILE, DRIVER_ALIAS_FILE, SEASON_FILE);
...
compile_opts.T_gated        = T_gated;


%% =========================================================
%  SECTION 5: COMPILE FROM COM FILES
%  Run this cell when you have new/changed COM .ld files.
%  Already-cached files are skipped automatically.
% =========================================================

COMPILE_DIR = fullfile(TOP_LEVEL_DIR, COM_SUBDIR);

cache = compile_event_data(COMPILE_DIR, TEAM_FILTER, channels, season, ...
                           driver_map, alias, compile_opts, MODE, worker_opts);
...
pit_summary = detect_tyre_changes(cache, driver_map);
...
upload_stats(cache, EVENT_NAME, TARGET, BATCH_SIZE, OVERWRITE, RUN_UPLOAD);
...
% ----------------------------------------------------------------------- %
%  LOAD_EVENT_CONFIG
%  Load all config files needed for the pipeline.
% ----------------------------------------------------------------------- %
function [channels, channel_rules, alias, driver_map, cfg, T_gated, season] = ...
        load_event_config(channels_file, event_alias_file, driver_alias_file, season_file)
...
% ----------------------------------------------------------------------- %
%  COMPILE_EVENT_DATA
%  Compile statistics from COM .ld files.  Supports serial and parallel
%  modes.  The compile directory should be the COM output folder written
%  by the VCS pipeline (e.g. <TOP_LEVEL_DIR>/COM).
% ----------------------------------------------------------------------- %
function cache = compile_event_data(compile_dir, team_filter, channels, season, ...
                                    driver_map, alias, compile_opts, mode, worker_opts)
    ...
    switch lower(mode)
        ...
        % ----------------------------------------------------------
            ...
            cache = smp_compile_event(compile_dir, team_filter, channels, ...
                                      season, driver_map, alias, compile_opts);

        % ----------------------------------------------------------
        case 'parallel'
        % ----------------------------------------------------------
            tmp_dir           = worker_opts.tmp_dir;
            n_workers         = worker_opts.n_workers;
            poll_interval_s   = worker_opts.poll_interval_s;
            timeout_s         = worker_opts.timeout_s;
            keep_workers_open = worker_opts.keep_workers_open;
            session_filter    = compile_opts.session_filter;
            ...
            if isfield(compile_opts, 'workshop') && compile_opts.workshop
            ...
            [min_lt, max_lt]               = smp_season_get(season, compile_opts.track);
            worker_cfg.test_mode           = false;
            worker_cfg.channels_to_extract = channels;
            worker_cfg.channel_rules       = compile_opts.channel_rules;
            worker_cfg.driver_map          = driver_map;
            worker_cfg.alias               = alias;
            worker_cfg.season              = season;
            worker_cfg.track               = compile_opts.track;
            worker_cfg.top_level_dir       = compile_dir;
...
% ----------------------------------------------------------------------- %
%  DETECT_TYRE_CHANGES
%  Infer tyre changes from cache by comparing wheel-sensor IDs lap-to-lap.
%  Only considers laps > MIN_LAP.  Returns a pit_summary-style table.
% ----------------------------------------------------------------------- %
function pit_summary = detect_tyre_changes(cache, driver_map)
...
% ----------------------------------------------------------------------- %
%  UPLOAD_STATS
%  Flatten cache stats and push to configured target.
% ----------------------------------------------------------------------- %
function upload_stats(cache, event_name, target, batch_size, overwrite, run_upload)
    ...
    switch lower(target)
        ...
        otherwise
            warning('Unknown upload target "%s" — skipping.', target);
    end
end
