...
%   'LoadOK'       logical       (default: true)
    ...
    % ------------------------------------------------------------------
    %  Apply LoadOK filter
    %  Skip if it would eliminate all rows (e.g. all entries failed on
    %  first compile but were later recovered via VCH recompute).
    % ------------------------------------------------------------------
    if opts.LoadOK && ismember('LoadOK', T.Properties.VariableNames)
        load_ok_mask = mask & logical(T.LoadOK);
        if any(load_ok_mask)
            mask = load_ok_mask;
        else
            fprintf('  [INFO] LoadOK filter skipped — no rows have LoadOK=true (all rows retained).\n');
        end
    end
    ...
    for t = 1:numel(team_keys)
        ...
        laps_cells   = cell(n_runs, 1);   % ← add with the other cell declarations
        ...
        for r = 1:n_runs
            ...
            if ~isempty(gk)
                ...
                if isfield(cache, 'laps') && isfield(cache.laps, gk)   % ← add this
                    laps_cells{r} = cache.laps.(gk);
                end
            end
        ...
        SMP_out.(matlab.lang.makeValidName(tk)).laps     = laps_cells; 
        fprintf('  %-8s  %d run(s)\n', tk, n_runs);
    end
