...
%         cands = {data.ADR_Acceleration_X, data.ADR_Acceleration_Y, ...
%          data.Acceleration_X_Filt, data.Acceleration_Y_Filt, ...
%          data.Throttle_Pedal,      data.Ground_Speed};
%         lens      = cellfun(@(c) numel(c.data), cands);
%         [~, refIdx] = max(lens);
%         ref       = cands{refIdx};
%         ref = longest_channel(data);
...
%         
%         averageLat = lat .* midCornerMask;   % lateral G
...
%     end
% 
%     if isfield(data, 'Wheel_Speed_Front_Right')  && ...
%         isfield(data, 'TPM1S_FR_WS_PRESS') 
...
%     %  Rear Left
%     % ------------------------------------------------------------------ %
%     if isfield(data, 'Wheel_Speed_Rear_Right')  && ...
%        isfield(data, 'TPM1S_RR_WS_PRESS') 
...
% 
%         % gamma = roll,  beta = pitch,  alpha = yaw (= 0)
%         FRH = data.Laser_Ride_Height_Front_Raw;
%         RRH = data.Laser_Ride_Height_Rear_Raw;
% 
%         % Align all signals to a common timebase (use whichever sensor is longer)
%         if length(FRH.data) >= length(RRH.data)
%             ref_ch   = FRH;
%             FRH_data = FRH.data(:);
%             RRH_data = align_to(RRH, FRH);
%         else
%             ref_ch   = RRH;
%             FRH_data = align_to(FRH, RRH);
%             RRH_data = RRH.data(:);
%         end
% 
%         gamma = deg2rad(align_to(data.roll_VCH,  ref_ch));  % roll
%         beta  = deg2rad(align_to(data.pitch_VCH, ref_ch));  % pitch
% 
%         % Reference from CAD 18/05/2026
%         % Corner translations [x, y, z] in mm
%         frontTx =  33.9;  frontTy =  427.74;  frontTz = 30.9;
%         rearTx  =  24.0;  rearTy  =  236.9;   rearTz  = 19.4;
% 
%         % z-row of rotation matrix (alpha = 0):
%         %   R_z = [-sin(b), cos(b)*sin(g), cos(b)*cos(g)]
%         % Applied to input point [0; 0; Z_sensor; 1] with corner translation [tx, ty, tz]:
%         %   result_z = cos(b)*cos(g)*Z_sensor + tz
%         % z-change at corner due to pitch/roll (relative to nominal position)
...
% 
%         data.FRH_FL_VCH = make_channel(FRH_data + dz_FL, ref_ch, 'mm', 'FRH_FL_VCH');
%         fprintf('  [+] FRH_FL_VCH\n');
%         data.FRH_FR_VCH = make_channel(FRH_data + dz_FR, ref_ch, 'mm', 'FRH_FR_VCH');
%         fprintf('  [+] FRH_FR_VCH\n');
%         data.RRH_RL_VCH = make_channel(RRH_data + dz_RL, ref_ch, 'mm', 'RRH_RL_VCH');
%         fprintf('  [+] RRH_RL_VCH\n');
%         data.RRH_RR_VCH = make_channel(RRH_data + dz_RR, ref_ch, 'mm', 'RRH_RR_VCH');
%         fprintf('  [+] RRH_RR_VCH\n');
% 
%     end   
%      
...
% 
%         data.longSlipFrontVCH = make_channel(longSlip, ref, 'ratio', 'long_Slip_Front_VCH');
...
%         end
% 
%         data.RL_SlipVCH = make_channel(rl_slip .* double(gate), ref_ch, '%', 'RL_Slip_VCH');
...
%     %  Requires: Laser_Ride_Height_Front_Raw, Laser_Ride_Height_Rear_Raw
%     %            info.manufacturer  (from motec_ld_info)
%     % ==================================================================
...
%             data.CLa_SCz_VCH = make_channel(aero.CLa_SCz, ref_ch, '-',  'CLa_SCz_VCH');
%             data.AB_FRT_VCH  = make_channel(aero.AB_FRT,  ref_ch, '%',  'AB_FRT_VCH');
%             data.CDa_SCx_VCH = make_channel(aero.CDa_SCx, ref_ch, '-',  'CDa_SCx_VCH');
%             data.EFF_VCH     = make_channel(aero.EFF,     ref_ch, '-',  'EFF_VCH');
%             fprintf('  [+] CLa_SCz_VCH  AB_FRT_VCH  CDa_SCx_VCH  EFF_VCH  (%s map, roll=0)\n', ...
%                 manufacturer);
% 
%             % Side force channels at roll = -2 deg
%             aero2 = aeroMapChannels(data.Laser_Ride_Height_Front_Raw, ...
%                                     data.Laser_Ride_Height_Rear_Raw, ...
%                                     manufacturer, AERO_MAP_DIR, -2);
%             if ~isempty(aero2)
%                 data.CSa_Scy_VCH  = make_channel(aero2.CSa_Scy,  ref_ch, '-', 'CSa_Scy_VCH');
%                 data.CSf_SCyF_VCH = make_channel(aero2.CSf_SCyF, ref_ch, '-', 'CSf_SCyF_VCH');
%                 data.CSr_SCyR_VCH = make_channel(aero2.CSr_SCyR, ref_ch, '-', 'CSr_SCyR_VCH');
...
%% Gating Section
    ...
    if isfield(data, 'Laser_Ride_Height_Front_Raw') && ...
       ...
       isfield(data, 'roll_VCH')
        ...
        gamma = deg2rad(align_to(data.roll_VCH,  ref_ch));
        beta  = deg2rad(align_to(data.pitch_VCH, ref_ch));
        ...
        data.FRH_FL_VCH = make_channel(FRH_data + dz_FL, ref_ch, 'mm', 'FRH_FL_VCH');  fprintf('  [+] FRH_FL_VCH\n');
        data.FRH_FR_VCH = make_channel(FRH_data + dz_FR, ref_ch, 'mm', 'FRH_FR_VCH');  fprintf('  [+] FRH_FR_VCH\n');
        data.RRH_RL_VCH = make_channel(RRH_data + dz_RL, ref_ch, 'mm', 'RRH_RL_VCH');  fprintf('  [+] RRH_RL_VCH\n');
        data.RRH_RR_VCH = make_channel(RRH_data + dz_RR, ref_ch, 'mm', 'RRH_RR_VCH');  fprintf('  [+] RRH_RR_VCH\n');
    end
...
% All channels are flagged write_to_ld=true by default (dec_places=2).
