...
%     % Guard against empty/non-scalar read (corrupt or truncated file)
%     if isscalar(event_ptr) && event_ptr > 0 && event_ptr < file_sz
...
%     % Guard against empty/non-scalar read
%     if ~isscalar(first_meta_ptr) || first_meta_ptr == 0 || first_meta_ptr >= file_sz
%         error('Invalid first channel metadata pointer: 0x%s', ...
%             mat2str(first_meta_ptr));
%     end
...
% 
%         % Guard against bad next_ptr before loop continues
...
% 
%         % Enhanced mode: skip channels not in filter
%         if enhanced_mode
...
%             if strcmpi(name_str, 'GPS Latitude') || strcmpi(name_str, 'GPS Longitude')
%                 fprintf('   [DEBUG] %s: datatype=%d  ecu_format=%d  dec=%d  mul=%d  scale=%d  offset=%d\n', ...
%                     name_str, datatype, ecu_format, dec_places, ch_mul, ch_scale, ch_offset);
%             end
%             switch datatype
%                 case 1
...
%                         phys = fread(fid, data_len, 'float32=>double', 0, 'l');
%                     else
%                         raw_data = fread(fid, data_len, 'int16=>double', 2, 'l');   % 2-byte skip = 4 bytes/sample total
%                         phys     = raw_data ./ (10^dec_places) + ch_offset;
%                     end
% 
%                 otherwise
...
%             end
% 
%             % Normalise distance channels to start at 0
...
% 
%                 case 4
%                     % Some dash-format files (this L180 export included) still
%                     % use native float32 for certain datatype-4 channels
%                     % (observed: GPS Latitude/Longitude) even when ecu_format
%                     % is false. dec_places >= 7 is not physically usable for
%                     % int16 (max range would be ±0.0032767) — treat it as the
%                     % signal that this channel is really float32, regardless
%                     % of the file-level ecu_format flag.
%                     if ecu_format || dec_places >= 7
%                         phys = fread(fid, data_len, 'float32=>double', 0, 'l');
%                     else
%                         raw_data = fread(fid, data_len, 'int16=>double', 2, 'l');
%                         phys     = raw_data ./ (10^dec_places) + ch_offset;
            ...
            switch datatype
                ...
                case 4
                    if ecu_format
                        ...
                        % Confirmed via raw byte dump (GPS Latitude/Longitude
                        % in this L180 file): plain int32, same formula as
                        % case 3. The previous "int16 + 2-byte skip" guess was
                        % wrong — raw bytes read cleanly as int32 (small,
                        % slowly-changing values consistent with GPS deltas
                        % relative to a reference point, not absolute lat/long).
                        raw_data = fread(fid, data_len, 'int32=>double', 0, 'l');
                        if ch_scale ~= 0 && ch_mul ~= 0
                            phys = raw_data .* (ch_mul / ch_scale) ./ (10^dec_places) + ch_offset;
                        else
