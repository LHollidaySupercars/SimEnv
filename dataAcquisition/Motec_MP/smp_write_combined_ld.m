...
%      fields, and write the full record back in-place.
    ...
    %% -- 2. Build ECU + L180 channel list for ld_add_channel -------------
    all_fields = fieldnames(merged);
    ecu_fields  = all_fields(strncmp(all_fields, 'ecu_',  4));
    l180_fields = all_fields(strncmp(all_fields, 'l180_', 5));

    fprintf('  ECU  channels to append: %d\n', numel(ecu_fields));
    fprintf('  L180 channels to append: %d\n', numel(l180_fields));

    % Build a unified list: ECU channels with 'ECU.' prefix, L180 with 'L180.' prefix
    prefix_map = [repmat({'ECU.'},  numel(ecu_fields),  1), ecu_fields; ...
                  repmat({'L180.'}, numel(l180_fields), 1), l180_fields];

    ecu_list = {};
    for k = 1:size(prefix_map, 1)
        append_prefix = prefix_map{k, 1};
        fn            = prefix_map{k, 2};
        ch = merged.(fn);
        ...
        new_ch.name        = str_to_name([append_prefix ch.raw_name], MAX_NAME);
    ...
    fprintf('\n=== Appending %d channel(s) via ld_add_channel ===\n', numel(ecu_list));
    if isempty(ecu_list)
        [ok, msg] = copyfile(dash_file, output_file, 'f');
        if ~ok, error('copyfile failed: %s', msg); end
        fprintf('  No channels to append -- copied dash file only.\n');
    else
    ...
    for i = 1:n_dash
        ...
        % Skip if already prefixed (e.g. channel was renamed in a prior run)
        if startsWith(orig_name, 'Dash.')
            fprintf('  [SKIP] Already prefixed: %s\n', orig_name);
    ...
    fprintf('  Total: %d Dash + %d %s + %d session channels.\n', ...
        n_dash, numel(ecu_list), append_label, numel(fieldnames(session_meta)));
    fprintf('  Output: %s\n', output_file);
end
