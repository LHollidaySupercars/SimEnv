...
%   0x0694  team_name     char[64]  e.g. "Triple Eight Race Engineering"
    ...
    info.run        = fstr(hdr, 0x624,  32);
    info.team_name  = fstr(hdr, 0x694,  64);
    info.venue_raw   = info.venue;
    info.session_raw = info.session;
    ...
    % ------------------------------------------------------------------ %
    %  Secondary "correction" block
    %  Confirmed via hex analysis (E07_TSV files): a second venue/session
    %  pair sits ~0x19BDA7 into the file, preceded by a 16-byte signature
    %  (uint32 x4 = [100 56 4 1]). This appears to hold a corrected value
    %  when the session was renamed/edited after initial logging — the
    %  primary header field at 0x5E4 is NOT updated in that case, so this
    %  block, when present, is the more trustworthy source.
    %  Located by signature search (not fixed offset) since the exact
    %  offset drifts by a byte or two depending on earlier variable-length
    %  header content.
    % ------------------------------------------------------------------ %
    info.venue_corrected   = '';
    info.session_corrected = '';
    [info.venue_corrected, info.session_corrected] = find_correction_block(filepath);

    if ~isempty(info.venue_corrected)
        info.venue = info.venue_corrected;
    end
    if ~isempty(info.session_corrected)
        info.session = info.session_corrected;
    end
    ...
    if verbose
        ...
        fprintf('Venue:        %s\n', info.venue);
        fprintf('Session:      %s\n', info.session);
        if ~isempty(info.venue_corrected) || ~isempty(info.session_corrected)
            fprintf('  (correction block found — raw header had venue="%s" session="%s")\n', ...
                info.venue_raw, info.session_raw);
        end
        fprintf('Run:          %s\n', info.run);
        fprintf('Year:         %s\n', info.year);
    end
...
function [venue_str, session_str] = find_correction_block(filepath)
% Search for the secondary venue/session correction block, identified by
% a 16-byte signature (uint32 x4 = [100 56 4 1]) confirmed via hex
% analysis. Venue field starts 48 bytes after the signature start,
% session field 112 bytes after (both 64-byte, null-padded ASCII).
%
% Scan is capped to the first SCAN_CAP bytes of the file — the block has
% been observed around 0x19BDA7 (~1.69MB in) across files from 7MB to
% 10.8MB, so this leaves generous margin without reading huge files
% fully into memory.
    venue_str = ''; session_str = '';

    SCAN_CAP    = 4 * 1024 * 1024;  % 4 MB
    VENUE_REL   = 48;
    SESSION_REL = 112;
    FIELD_LEN   = 64;
    ...
    venue_str   = extract_field_local(raw, sig_off + VENUE_REL,   FIELD_LEN, n);
    session_str = extract_field_local(raw, sig_off + SESSION_REL, FIELD_LEN, n);
end

function str = extract_field_local(raw, start_off, len, n)
    idx  = start_off + 1;
    last = min(idx + len - 1, n);
    if idx > n, str = ''; return; end
    seg = raw(idx:last);
    nul = find(seg == 0, 1);
    if ~isempty(nul), seg = seg(1:nul-1); end
    if isempty(seg) || any(seg < 32 | seg > 126)
