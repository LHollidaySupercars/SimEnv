...
%   outlier_threshold        — scalar multiplier
    ...
    for p = 1:numel(plots)
        ...
        try
            ...
            % ---- ADD: lap type filter ----
            if isfield(pd, 'lap_type_filter') && ~isempty(pd.lap_type_filter)
                allowed_types = strtrim(strsplit(pd.lap_type_filter, ';'));
                plot_run_list = apply_lap_type_filter(plot_run_list, allowed_types);
            end
...
%                     figs{p} = make_psd(plot_run_list, pd, colour_cfg, driver_map, opts);
        ...
        if ~isempty(figs{p})
            if ~use_subplot && ~strcmpi(pd.type, 'timeseries_align')
            ...
            if isfield(pd,'highlight_outliers') && pd.highlight_outliers && ...
                ...
                draw_outlier_highlights(figs{p}, pd, plot_run_list, colour_cfg, driver_map);
            end
        end
...
function entry = build_entry_stream(node, r, tk, driver_map)
    ...
    if isempty(entry.manufacturer)
        entry.manufacturer = driver_map.(node.meta.Driver(r)).manufacturer;
    end
    ...
    entry.tla          = resolve_tla(entry.driver, driver_map);
    entry.stats        = stats_s;
    ...
    % Pull lap metadata from node if available
    if isfield(node, 'laps') && numel(node.laps) >= r
        entry.laps_meta = node.laps{r};
    else
        entry.laps_meta = [];
    end
...
function col = resolve_colour(entry, colour_mode, colour_cfg, driver_map)
    switch lower(colour_mode)
        ...
        case 'coolsuit'
            dryIce = {'8','14','96','3','7','17','38','18','20','6','55','1','2'};
            chillOut = {'10','4'};
            chillOutGenesis = {'9', '99', '777','31', '19', '26','11', '88', '888'};
            existsDryIce = any(strcmp(entry.car, dryIce))           
            existsChillOut = any(strcmp(entry.car, chillOut))
            existsChillOutGenesis = any(strcmp(entry.car,chillOutGenesis))
            
            if existsDryIce
                col = [255, 0, 189]/255;
            elseif existsChillOut
                col = [135, 233, 17]/255;
            elseif existsChillOutGenesis
                col = [225, 24, 69]/255;
            else
                keyboard
            end
        case 'custom'
            dist_0   = {'11', '88', '888'};
            dist_50  = {'1', '2', '19', '26'};
            dist_100 = {'777', '31', '10', '4','3', '7'};;
            dist_200 = {'99','9'};
            dist_300 ={'8','14','96'};
            dist_400 = {'17', '38', '18', '20', '6', '55', };
             

            existDist_0    = any(strcmp(entry.car, dist_0  ))
            existDist_50   = any(strcmp(entry.car, dist_50 ))
            existDist_100  = any(strcmp(entry.car, dist_100))
            existDist_200  = any(strcmp(entry.car, dist_200))
            existDist_300  = any(strcmp(entry.car, dist_300))
            existDist_400  = any(strcmp(entry.car, dist_400))
            if existDist_0 
                col = [13, 245, 233]/255;
            elseif existDist_50 
                col = [255, 0, 189]/255;
            elseif existDist_100
                col = [135, 233, 17]/255
            elseif existDist_200
                col = [225, 24, 69]/255
            elseif existDist_300
                col = [0, 87, 233]/255
            elseif existDist_400
                col = [137, 49, 239]/255
            else
                keyboard
            end
...
function apply_legend(ax, handles, labels, opts)
    ...
    legend(ax, handles(valid), labels(valid), ...
        'Location','best','FontSize',fs-1,'Box','off','Interpreter','none','TextColor','black');
end
...
function fig = make_scatter(run_list, pd, colour_cfg, driver_map, opts, SHAPES, ax_in)
    ...
    % --- FIX 1: detect Lap Time as a keyword so it never goes through math ---
    is_lap_time_axis = any(strcmpi(pd.x_axis, {'Lap_Time','Lap Time','laptime','lap_time'}));
    ...
    for r = 1:numel(run_list)
        ...
        for yi = 1:numel(pd.y_channels)
            ...
            lap_times = entry.stats.(y_field).lap_times;   % always available
            ...
            if is_falling
                ...
                x_vals = 1:numel(y_vals);
                x_lbl  = 'Rank';

            elseif is_lap_time_axis
                % --- FIX 1: use lap_times directly, no math applied ---
                x_vals = lap_times;
                x_lbl  = 'Lap Time (s)';
...
function T = make_table2csv(run_list, pd, driver_map, opts)

    is_lap_time_axis = any(strcmpi(pd.x_axis, {'Lap_Time','Lap Time','laptime','lap_time'}));
    is_falling       = contains(lower(pd.name),'falling') || contains(lower(pd.x_axis),'falling');

    % resolve the filter label once upfront
    if isfield(pd, 'lap_type_filter') && ~isempty(pd.lap_type_filter)
        filter_label = strtrim(pd.lap_type_filter);
    else
        filter_label = 'none';
    end
    ...
    row_y             = [];
    row_ych           = {};
    row_lap_type_filter = {};   % <-- NEW

    for r = 1:numel(run_list)
        entry = run_list(r);

        sess_lbl = get_field_safe(entry, 'session');
        drv_lbl  = get_field_safe(entry, 'driver');
        car_lbl  = get_field_safe(entry, 'car');
        ...
        for yi = 1:numel(pd.y_channels)
            ...
            y_vals    = local_apply_math(entry.stats.(y_field), pd.math_fn);
            lap_nums  = entry.stats.(y_field).lap_numbers;
            lap_times = entry.stats.(y_field).lap_times;
            valid     = isfinite(y_vals);
            y_vals    = y_vals(valid);
            lap_nums  = lap_nums(valid);
            lap_times = lap_times(valid);
            ...
            n = numel(y_vals);
            row_plotName        = [row_plotName;        repmat({pd.name},      n, 1)]; %#ok
            row_session         = [row_session;         repmat({sess_lbl},     n, 1)]; %#ok
            row_driver          = [row_driver;          repmat({drv_lbl},      n, 1)]; %#ok
            row_car             = [row_car;             repmat({car_lbl},      n, 1)]; %#ok
            row_lap             = [row_lap;             lap_nums(:)];                   %#ok
            row_x               = [row_x;               x_vals(:)];                    %#ok
            row_y               = [row_y;               y_vals(:)];                    %#ok
            row_ych             = [row_ych;             repmat({y_ch},         n, 1)]; %#ok
            row_lap_type_filter = [row_lap_type_filter; repmat({filter_label}, n, 1)]; %#ok
        end
    end

    T = table(row_plotName, row_session, row_driver, row_car, ...
              row_lap, row_x, row_y, row_ych, row_lap_type_filter, ...
              'VariableNames', {'PlotName','Session','Driver','Car', ...
                                'Lap','X','Y','YChannel','LapTypeFilter'});

    % --- filename: plotName_session_laptype.csv ---
    safe_name = regexprep(pd.name, '[^\w]', '_');
    sessions  = unique(row_session);
    sess_str  = '';
    if ~isempty(sessions) && ~isempty(sessions{1})
        sess_str = ['_' strjoin(sessions, '_')];
    end
    lap_type_str = '';
    if ~strcmp(filter_label, 'none')
        lap_type_str = ['_' regexprep(filter_label, '[^\w]', '_')];
    end

    csv_fname = sprintf('%s%s%s.csv', safe_name, sess_str, lap_type_str);
    csv_path  = fullfile(pwd, csv_fname);
    try
        writetable(T, csv_path);
        fprintf('[make_table2csv] %d rows -> %s\n', height(T), csv_path);
    catch ME
        warning('[make_table2csv] Could not write CSV: %s', ME.message);
    end
end

function val = get_field_safe(s, field)
    if isfield(s, field), val = char(s.(field)); else, val = ''; end
end

% -----------------------------------------------------------------------
function val = get_meta(meta, field)
...
%     use_shapes    = strcmpi(pd.differentiator, 'shapes');
%     is_falling    = contains(lower(pd.name),'falling') || contains(lower(pd.x_axis),'falling');
%     is_lapTimes = contains(lower(pd.x_axis), 'Lap Time')
%     
%     leg_h = []; leg_l = {}; leg_seen = {};
%     x_lbl = 'Lap Number';
% 
...
%                       make scatter trace
%========================================================================
% function fig = make_scatter_trace(run_list, pd, colour_cfg, driver_map, opts, SHAPES, ax_in)
% % MAKE_SCATTER_TRACE  Plots time-series trace data for the best lap.
% %
% % X-axis behaviour:
% %   - pd.x_axis = 'lap' / 'lapnumber' / empty  → distance (default)
% %   - pd.x_axis = channel name                  → that channel's trace data
% %
% % Z-axis / 3D:
% %   - pd.z_axis non-empty                       → scatter3 using z channel trace
% %
% % Supports single-multichan colouring, secondary y-axis, shapes differentiator.
% 
%     if nargin < 7, ax_in = []; end
%     [fig, ax] = new_fig(pd, opts, ax_in);
%     fs = get_opt(opts, 'font_size', 11);
...
%     has_x_ch  = isfield(pd, 'x_axis') && ~isempty(pd.x_axis) && ~is_keyword(pd.x_axis);
%     x_lbl     = 'Distance (m)';
%     if has_x_ch, x_lbl = pd.x_axis; end
% 
%     % Determine z-axis / 3D mode
%     has_z     = isfield(pd, 'z_axis') && ~isempty(pd.z_axis);
%     use_3d    = has_z;
% 
%     % Single driver/dataset with multiple y-channels → colour by channel
%     single_multichan = (numel(pd.y_channels) > 1) && ...
%         (numel(unique({run_list.driver})) == 1);
%     CHAN_COLOURS = [
%         0.12  0.47  0.71;   % blue
...
%             y_field = sanitise_fn(y_ch);
% 
%             % Per-channel colour override
%             if single_multichan
%                 col = CHAN_COLOURS(mod(yi-1, size(CHAN_COLOURS,1))+1, :);
%             end
% 
%             % Secondary y-axis switching (2D only)
%             if ~use_3d
%                 if use_secondary && yi == 2
%                     yyaxis(ax, 'right');
...
%             end
% 
%             % ---- Extract best lap traces ----
%             d_dist  = [];   % native distance axis
%             v_y     = [];   % y channel
%             v_x_ch  = [];   % x channel (if specified)
%             v_z     = [];   % z channel (if 3D)
% 
%             if strcmp(entry.mode, 'stream')
%                 tr = entry.traces;
%                 if isempty(tr) || ~isstruct(tr) || tr.n_traces == 0
%                     fprintf('  [WARN] No traces for %s — skipping.\n', entry.driver);
%                     continue;
%                 end
...
% 
%                 d_dist = tr.(y_field)(best_k).dist(:);
%                 v_y    = tr.(y_field)(best_k).data(:);
% 
%                 if has_x_ch
%                     xf = sanitise_fn(pd.x_axis);
%                     if isfield(tr, xf)
%                         v_x_ch = tr.(xf)(best_k).data(:);
%                     else
%                         fprintf('  [WARN] x_axis trace "%s" not found for %s — using distance.\n', pd.x_axis, entry.driver);
%                     end
%                 end
% 
%                 if has_z
%                     zf = sanitise_fn(pd.z_axis);
%                     if isfield(tr, zf)
%                         v_z = tr.(zf)(best_k).data(:);
%                     else
%                         fprintf('  [WARN] z_axis trace "%s" not found for %s — falling back to 2D.\n', pd.z_axis, entry.driver);
%                     end
%                 end
% 
%             else
...
%                 [~, best] = min(lap_times);
% 
%                 yf = find_ch_field(laps(best).channels, y_ch);
%                 if isempty(yf)
%                     fprintf('  [WARN] Channel "%s" not found for %s\n', y_ch, entry.driver);
%                     continue;
%                 end
%                 ch_b = laps(best).channels.(yf);
%                 if ~isfield(ch_b, 'dist') || ~isfield(ch_b, 'data')
%                     fprintf('  [WARN] .dist/.data missing for %s — delete cache and recompile.\n', entry.driver);
%                     continue;
%                 end
%                 d_dist = ch_b.dist(:);
%                 v_y    = ch_b.data(:);
% 
%                 if has_x_ch
%                     xf = find_ch_field(laps(best).channels, pd.x_axis);
%                     if ~isempty(xf)
%                         v_x_ch = laps(best).channels.(xf).data(:);
%                     else
%                         fprintf('  [WARN] x_axis "%s" not found for %s — using distance.\n', pd.x_axis, entry.driver);
%                     end
%                 end
% 
%                 if has_z
%                     zf = find_ch_field(laps(best).channels, pd.z_axis);
%                     if ~isempty(zf)
%                         v_z = laps(best).channels.(zf).data(:);
%                     else
%                         fprintf('  [WARN] z_axis "%s" not found for %s — falling back to 2D.\n', pd.z_axis, entry.driver);
%                     end
%                 end
%             end
% 
%             if isempty(d_dist) || isempty(v_y), continue; end
% 
%             % ---- Resolve x vector ----
%             if has_x_ch && ~isempty(v_x_ch)
%                 n_pts = min(numel(v_x_ch), numel(v_y));
%                 x_vec = v_x_ch(1:n_pts);
%                 y_vec = v_y(1:n_pts);
%             else
%                 n_pts = min(numel(d_dist), numel(v_y));
%                 x_vec = d_dist(1:n_pts);
%                 y_vec = v_y(1:n_pts);
%             end
% 
%             % ---- Plot ----
%             if use_3d && ~isempty(v_z)
%                 n_pts = min(numel(x_vec), numel(v_z));
%                 x_vec = x_vec(1:n_pts);
%                 y_vec = y_vec(1:n_pts);
%                 z_vec = v_z(1:n_pts);
%                 h = scatter3(ax, x_vec, y_vec, z_vec, 10, col, 'filled', ...
%                     'MarkerFaceAlpha', 0.7);
%                 zlabel(ax, pd.z_axis, 'FontSize', fs, 'Interpreter','none');
%                 view(ax, 3);
%             else
%                 if use_shapes && yi <= numel(SHAPES)
%                     h = plot(ax, x_vec, y_vec, ['-', SHAPES{yi}], ...
%                         'Color', col, 'LineWidth', lw, ...
%                         'MarkerSize', 4, 'MarkerFaceColor', col, ...
%                         'MarkerEdgeColor', col*0.7);
%                 else
%                     h = plot(ax, x_vec, y_vec, '-', ...
%                         'Color', col, 'LineWidth', lw);
%                 end
%             end
% 
...
%                 leg_h(end+1) = h; %#ok
%                 leg_l{end+1} = leg_txt; %#ok
%                 leg_seen{end+1} = leg_key; %#ok
%             end
%         end
%     end
% 
%     if use_secondary && ~use_3d, yyaxis(ax, 'left'); end
% 
%     xlabel(ax, x_lbl, 'FontSize', fs, 'Interpreter','none');
%     if ~use_secondary
%         ylabel(ax, strjoin(pd.y_channels,' / '), 'FontSize', fs, 'Interpreter','none');
%     end
%     apply_legend(ax, leg_h, leg_l, opts);
%     if use_shapes && numel(pd.y_channels) > 1 && ~use_3d
%         add_shape_key(fig, pd.y_channels, SHAPES, fs);
%     end
%     apply_axis_limits(ax, pd);
% end
...
% %
% % GPS track map:
% %   - When x_axis contains 'gps'/'lat'/'lon', rotates trace to start at
% %     beacon crossing so the track plots as one continuous loop.
% %
% % Supports single-multichan colouring, secondary y-axis, shapes differentiator.
% 
%     if nargin < 7, ax_in = []; end
...
%     use_3d = has_z;
% 
%     % GPS track map detection — rotate trace instead of NaN-suppressing
%     is_gps_plot = has_x_ch && (contains(lower(pd.x_axis), 'gps') || ...
%                                contains(lower(pd.x_axis), 'lat') || ...
%                                contains(lower(pd.x_axis), 'lon'));
% 
%     % Single driver/dataset with multiple y-channels → colour by channel
%     single_multichan = (numel(pd.y_channels) > 1) && ...
%         (numel(unique({run_list.driver})) == 1);
...
%             end
% 
%              % ---- GPS inter-lap reoffset ----
%             % If MoTeC resets the GPS reference mid-session, laps will plot
%             % in different locations. Detect this by comparing each lap's
%             % mean position to lap 1 and subtract the offset.
%             if is_gps_plot && strcmp(entry.mode, 'stream')
%                 xf_gps = sanitise_fn(pd.x_axis);
%                 yf_gps = y_field;
%                 if isfield(tr, xf_gps) && isfield(tr, yf_gps)
%                     % Reference = mean position of best lap
%                     ref_x = nanmean(tr.(xf_gps)(best_k).data);
%                     ref_y = nanmean(tr.(yf_gps)(best_k).data);
%                     % Check all laps for large offset vs reference
%                     for k2 = 1:tr.n_traces
%                         if k2 == best_k, continue; end
%                         mx = nanmean(tr.(xf_gps)(k2).data);
%                         my = nanmean(tr.(yf_gps)(k2).data);
%                         step_x = abs(mx - ref_x);
%                         step_y = abs(my - ref_y);
%                         % Threshold: if mean shift > 10x typical lap spread
%                         spread_x = nanstd(tr.(xf_gps)(best_k).data);
%                         spread_y = nanstd(tr.(yf_gps)(best_k).data);
%                         if step_x > 10 * spread_x || step_y > 10 * spread_y
%                             fprintf('  [GPS reoffset] Lap %d offset by [%.5f, %.5f] — correcting.\n', ...
%                                 tr.lap_numbers(k2), mx - ref_x, my - ref_y);
%                             tr.(xf_gps)(k2).data = tr.(xf_gps)(k2).data - (mx - ref_x);
%                             tr.(yf_gps)(k2).data = tr.(yf_gps)(k2).data - (my - ref_y);
%                         end
%                     end
%                     % Re-extract best lap after correction
%                     v_x_ch = tr.(xf_gps)(best_k).data(:);
%                     v_y    = tr.(yf_gps)(best_k).data(:);
%                 end
%             end
% 
%             % ---- GPS lap rotation ----
% 
%             % The lap beacon splits the trace mid-track. Find the largest
%             % positional jump (beacon crossing) and rotate the arrays so
%             % the trace starts there — giving one continuous loop.
%             if is_gps_plot && numel(x_vec) > 10 && numel(y_vec) > 10
%                 dx_g   = diff(x_vec);
%                 dy_g   = diff(y_vec);
%                 step_g = sqrt(dx_g.^2 + dy_g.^2);
%                 [~, beacon_idx] = max(step_g);
%                 rot = beacon_idx + 1;
%                 if rot > 1 && rot <= numel(x_vec)
%                     x_vec = [x_vec(rot:end); x_vec(1:rot-1)];
%                     y_vec = [y_vec(rot:end); y_vec(1:rot-1)];
%                     if use_3d && ~isempty(v_z) && numel(v_z) >= numel(x_vec)
%                         v_z = [v_z(rot:end); v_z(1:rot-1)];
%                     end
%                 end
%             end
% 
%             % ---- Non-GPS beacon jump suppression ----
%             % Inserts NaN breaks where consecutive point distance exceeds
%             % 50x the median step — catches the start/finish artifact
%             % on non-GPS distance/channel plots.
%             if ~is_gps_plot && numel(x_vec) > 2 && numel(y_vec) > 2
%                 dx      = diff(x_vec);
%                 dy      = diff(y_vec);
%                 step_d  = sqrt(dx.^2 + dy.^2);
%                 thresh  = 50 * median(step_d(step_d > 0));
%                 jumps   = find(step_d > thresh);
%                 if ~isempty(jumps)
%                     offset = 0;
%                     for ji = 1:numel(jumps)
%                         ins   = jumps(ji) + offset + 1;
%                         x_vec = [x_vec(1:ins-1); NaN; x_vec(ins:end)];
%                         y_vec = [y_vec(1:ins-1); NaN; y_vec(ins:end)];
%                         if use_3d && ~isempty(v_z)
%                             v_z = [v_z(1:ins-1); NaN; v_z(ins:end)];
%                         end
%                         offset = offset + 1;
%                     end
%                 end
...
%                 marker = 'o';
%                 if use_shapes && yi <= numel(SHAPES), marker = SHAPES{yi}; end
%                 h = scatter(ax, x_vec, y_vec, 2, col, marker, 'filled', ...
%                     'MarkerFaceAlpha', 0.6);
%             end
% 
%             % ---- Legend ----
...
%     end
% 
%     % GPS track maps: equal aspect ratio, clean axis labels
%     if is_gps_plot
%         axis(ax, 'equal');
%         xlabel(ax, 'GPS Latitude (relative)', 'FontSize', fs, 'Interpreter','none');
%         ylabel(ax, 'GPS Longitude (relative)', 'FontSize', fs, 'Interpreter','none');
%     end
% 
%     apply_axis_limits(ax, pd);
% end
...
%   - pd.polyfit_degree     integer >= 1 (1=linear, 2=quad, 3=cubic, etc.)
%                           Omit or set 0 to disable.
%   - pd.polyfit_per_driver true  = one fit line per driver/series (colour-matched)
%                           false = single combined fit (black dashed)
%
% Supports single-multichan colouring, secondary y-axis, shapes differentiator.
...
%     % ---- Polynomial fit overlay ----
%     pd.polyfit_degree = 1;
%     % if isfield(pd, 'polyfit_degree') && ~isempty(pd.polyfit_degree)
%         poly_degree = pd.polyfit_degree;
%     % end
%     per_driver_fit = isfield(pd, 'polyfit_per_driver') && pd.polyfit_per_driver;
% 
%     if poly_degree > 0 && ~is_gps_plot
%         scatter_objs = findobj(ax, 'Type', 'Scatter');
%         per_driver_fit = true
%         if per_driver_fit
%             % One fit line per scatter series, colour-matched to that series
%             for si = 1:numel(scatter_objs)
%                 so = scatter_objs(si);
%                 xd = double(so.XData(:));
%                 yd = double(so.YData(:));
%                 ok = isfinite(xd) & isfinite(yd);
%                 if sum(ok) < poly_degree + 1, continue; end
%                 p_cf = polyfit(xd(ok), yd(ok), poly_degree);
%                 xq   = linspace(min(xd(ok)), max(xd(ok)), 500)';
%                 yq   = polyval(p_cf, xq);
%                 sc   = so.CData;
%                 if size(sc, 1) > 1, sc = mean(sc, 1); end  % fallback if per-point colours
%                 fprintf('  [polyfit] si=%d  npts=%d  colour=[%.2f %.2f %.2f]\n', ...
%                     si, sum(ok), sc(1), sc(2), sc(3));
%                 plot(ax, xq, yq, '-', 'Color', sc, 'LineWidth', 1.5, ...
%                     'HandleVisibility', 'off');
%             end
%         else
%             % Single combined fit across all series — black dashed
%             all_x = []; all_y = [];
%             for si = 1:numel(scatter_objs)
%                 xd = double(scatter_objs(si).XData(:));
%                 yd = double(scatter_objs(si).YData(:));
%                 ok = isfinite(xd) & isfinite(yd);
%                 all_x = [all_x; xd(ok)]; %#ok<AGROW>
%                 all_y = [all_y; yd(ok)]; %#ok<AGROW>
%             end
%             if numel(all_x) >= poly_degree + 1
%                 p_cf = polyfit(all_x, all_y, poly_degree);
%                 xq   = linspace(min(all_x), max(all_x), 500)';
%                 yq   = polyval(p_cf, xq);
%                 plot(ax, xq, yq, 'k--', 'LineWidth', 1.8, ...
%                     'HandleVisibility', 'off');
%             end
%         end
%     end
...
function fig = trace_scatter_all(run_list, pd, colour_cfg, driver_map, opts, SHAPES, ax_in)
% TRACE_SCATTER_ALL  Plots scatter of trace data across ALL valid laps.
%
% X-axis behaviour:
%   - pd.x_axis = 'lap' / 'lapnumber' / empty  → distance (default)
%   - pd.x_axis = channel name                  → that channel's trace data
...
%
% Polynomial fit overlay:
%   - Hardcoded: degree=1, per-driver, colour-matched.
%
% Supports single-multichan colouring, secondary y-axis, shapes differentiator.
    ...
    for r = 1:numel(run_list)
        ...
        for yi = 1:numel(pd.y_channels)
            ...
            if strcmp(entry.mode, 'stream')
                ...
                for ki = 1:numel(valid_k)
                    k2     = valid_k(ki);
                    d_dist = [d_dist; tr.(y_field)(k2).dist(:)]; %#ok
                    v_y    = [v_y;    tr.(y_field)(k2).data(:)]; %#ok

                    if has_x_ch
                        xf = sanitise_fn(pd.x_axis);
                        if isfield(tr, xf)
                            v_x_ch = [v_x_ch; tr.(xf)(k2).data(:)]; %#ok
                        end
                    end
                ...
                for li = 1:numel(laps)
                    yf = find_ch_field(laps(li).channels, y_ch);
                    if isempty(yf), continue; end
                    ch_b = laps(li).channels.(yf);
                    if ~isfield(ch_b, 'dist') || ~isfield(ch_b, 'data'), continue; end
                    d_dist = [d_dist; ch_b.dist(:)]; %#ok
                    v_y    = [v_y;    ch_b.data(:)]; %#ok

                    if has_x_ch
                        xf = find_ch_field(laps(li).channels, pd.x_axis);
                        if ~isempty(xf)
                            v_x_ch = [v_x_ch; laps(li).channels.(xf).data(:)]; %#ok
                        end
                    end

                    if has_z
                        zf = find_ch_field(laps(li).channels, pd.z_axis);
                        if ~isempty(zf)
                            v_z = [v_z; laps(li).channels.(zf).data(:)]; %#ok
                        end
                    end
                end
            end
            ...
            % ---- Near-zero value filter (gLong channels only) ----
            if ~is_gps_plot && any(strcmp(y_ch, {'Braking_gLong', 'Entry_gLong'}))
                keep = x_vec > 50 & y_vec < -0.1;
                x_vec = x_vec(keep);
                y_vec = y_vec(keep);
                if ~isempty(v_z) && numel(v_z) == numel(keep)
                    v_z = v_z(keep);
                end
            end
            % ---- Non-GPS beacon jump suppression ----
    ...
    % ---- Polynomial fit overlay (degree=1, per-driver, colour-matched) ----
    poly_degree = 1;
    if poly_degree > 0 && ~is_gps_plot
        scatter_objs = findobj(ax, 'Type', 'Scatter');
        for si = 1:numel(scatter_objs)
            ...
            xd = double(so.XData(:));
            yd = double(so.YData(:));
            ok = isfinite(xd) & isfinite(yd) & xd >= 200;
            if sum(ok) < poly_degree + 1, continue; end
            p_cf = polyfit(xd(ok), yd(ok), poly_degree);
            xq   = linspace(min(xd(ok)), max(xd(ok)), 500)';
            yq   = polyval(p_cf, xq);
            sc   = so.CData;
            if size(sc, 1) > 1, sc = mean(sc, 1); end
            plot(ax, xq, yq, '-', 'Color', sc, 'LineWidth', 1.5, ...
                'HandleVisibility', 'off');
        end
    end
...
function fig = make_timeseries(run_list, pd, colour_cfg, driver_map, opts, ...
    ...
    % Edge case: single dataset, multiple y-channels → colour by channel index
    single_multichan = (numel(pd.y_channels) > 1) && ...
        (numel(unique({run_list.driver})) == 1);
    CHAN_COLOURS = [
        0.12  0.47  0.71;   % blue
    ...
    for r = 1:numel(run_list)
        ...
        if strcmp(entry.mode, 'stream')
            ...
            for yi = 1:numel(pd.y_channels)
                ...
                % Override colour per channel when single dataset + multi-channel
                if single_multichan
                    col = CHAN_COLOURS(mod(yi-1, size(CHAN_COLOURS,1))+1, :);
                end
                ...
                % Select best lap — exclude lap 0 (pitlane/install lap in non-race sessions)
                valid_k = find(tr.lap_numbers >= -1 & isfinite(tr.lap_times) & tr.lap_times > 40);
                warning('using static time call, need robust saturation for lap times ')
                if isempty(valid_k)
                    valid_k = 1:tr.n_traces;   % fallback: use all
                end
                [~, rel_best] = min(tr.lap_times(valid_k));
                ...
                if single_multichan
                    leg_key = y_ch;
                    leg_txt = sprintf('%s  [%s]', y_ch, lap_str);
                elseif numel(pd.y_channels) > 1
                    lbl     = build_label(entry, pd, 1, driver_map);
                    leg_key = lbl;
                    leg_txt = sprintf('%s  (%s)  [%s]  [%s]', lbl, drv_str, pd.y_channels{1}, lap_str);
                else
                    lbl     = build_label(entry, pd, 1, driver_map);
                    leg_key = lbl;
                    leg_txt = sprintf('%s  (%s)  [%s]', lbl, drv_str, lap_str);
                end
...
% Plots fastest-valid-lap traces from entry.traces, aligned via peak-shift on raw data.
...
%
% "Fastest valid lap" = lowest lap_time that is still >= MIN_LAP_TIME, guarding
% against short/glitch entries (out-laps, partial laps) sorted into the traces array.
    ...
    MIN_LAP_TIME = get_opt(opts, 'min_lap_time', 45);
    ...
    % ---- Validate: each run must have a valid lap (>= MIN_LAP_TIME) with the first y-channel ----
    ...
    for r = 1:n
        ...
        li = pick_lap_idx(tr, MIN_LAP_TIME);
        if isempty(li), continue; end          % no lap >= MIN_LAP_TIME for this run
        lap_idx(r) = li;

        if ~isfield(tr.(y0_field)(li), 'dist') || isempty(tr.(y0_field)(li).dist)
            continue;
        end
        valid(r) = true;
    end
    ...
    if numel(valid_idx) < 2
        warning('make_timeseries_align: fewer than 2 runs with valid laps >= %gs for "%s".', ...
                MIN_LAP_TIME, pd.y_channels{1});
        return;
    end
    ...
    if ~isempty(align_ch)
        ...
        if isfield(tr_ref, align_field) && numel(tr_ref.(align_field)) >= ref_li
            d_ref = tr_ref.(align_field)(ref_li).dist(:);
            v_ref = tr_ref.(align_field)(ref_li).data(:);
        ...
        if isnan(ref_peak_dist)
            ...
            for r = valid_idx(2:end)
                tr = run_list(r).traces;
                li = lap_idx(r);
                if ~isfield(tr, align_field) || numel(tr.(align_field)) < li, continue; end
                d_raw = tr.(align_field)(li).dist(:);
                v_raw = tr.(align_field)(li).data(:);
                if ~isempty(align_win)
    ...
    for r = valid_idx
        ...
        for yi = 1:numel(pd.y_channels)
            ...
            if ~isfield(tr, yf) || numel(tr.(yf)) < li, continue; end
            d = tr.(yf)(li).dist(:);
            if ~isempty(d)
    ...
    for r = valid_idx
        ...
        for yi = 1:numel(pd.y_channels)
            ...
            if ~any(strcmp(leg_seen, lbl))
                leg_h(end+1) = h; %#ok
                lap_num_str = '';
                if isfield(tr, 'lap_numbers') && numel(tr.lap_numbers) >= li
                    lap_num_str = sprintf('Lap %d  |  ', tr.lap_numbers(li));
                end
                lap_time_str = '';
                if isfield(tr, 'lap_times') && numel(tr.lap_times) >= li
                    lap_time_str = sprintf('  [%s%.3fs]', lap_num_str, tr.lap_times(li));
                end
                if offsets_m(r) ~= 0
...
function vals = local_apply_math(stat, math_fn)
    switch lower(strtrim(math_fn))
        ...
        case 'change'
            if isfield(stat,'change'),  vals = stat.change;
            else, vals = stat.min - stat.max;  warning('local_apply_math:change not in stats.');  end
...
% per-lap math_fn value for each phase channel.
    ...
    [fig, ax] = new_fig(pd, opts, ax_in);
    fs = get_opt(opts, 'font_size', 11);
    if isempty(pd.x_axis)
%         PHASE_LABELS = pd.x_axis;
        PHASE_LABELS = {'Braking', 'Entry', 'Mid-Corner', 'Exit'};
    else
        PHASE_LABELS = eval(pd.x_axis);
    end
...
function run_list = apply_lap_type_filter(run_list, allowed_types)
    for i = 1:numel(run_list)
        if ~isfield(run_list(i), 'laps_meta') || isempty(run_list(i).laps_meta)
            continue;
        end

        lm   = run_list(i).laps_meta;
        keep = ismember(lm.lap_type, allowed_types);

        % Filter laps_meta
        lm_fieldnames = fieldnames(lm);
        lm_temp = struct();
        for f = 1:numel(lm_fieldnames)
            lm_temp.(lm_fieldnames{f}) = lm.(lm_fieldnames{f})(keep);
        end
        run_list(i).laps_meta = lm_temp;

        % Filter stats to matching lap numbers
        kept_lap_nums = lm.lap_number(keep);
        if isfield(run_list(i), 'stats') && ~isempty(run_list(i).stats)
            run_list(i).stats = filter_stats_by_lap(run_list(i).stats, kept_lap_nums);
        end
    end
end


function stats = filter_stats_by_lap(stats, kept_lap_nums)
    ch_names = fieldnames(stats);
    for c = 1:numel(ch_names)
        ch = stats.(ch_names{c});
        if ~isstruct(ch) || ~isfield(ch, 'lap_numbers')
            continue;
        end
        keep_idx = ismember(ch.lap_numbers, kept_lap_nums);
        n_laps   = numel(ch.lap_numbers);
        fn = fieldnames(ch);
        for f = 1:numel(fn)
            v = ch.(fn{f});
            if isnumeric(v) && numel(v) == n_laps
                ch.(fn{f}) = v(keep_idx);
            elseif iscell(v) && numel(v) == n_laps
                ch.(fn{f}) = v(keep_idx);
            end
            % leave anything that isn't lap-length alone
        end
        stats.(ch_names{c}) = ch;
    end
end

function li = pick_lap_idx(tr, min_lap_time)
li = [];
if ~isfield(tr, 'lap_times') || isempty(tr.lap_times)
    li = 1;  % no lap_times info, fall back to index 1
    return;
end
ok = find(tr.lap_times >= min_lap_time);
if isempty(ok), return; end
[~, best] = min(tr.lap_times(ok));
li = ok(best);
end