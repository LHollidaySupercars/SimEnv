...
%   cache_in        (optional) already-loaded cache struct — skips disk load
    ...
    % ------------------------------------------------------------------
    %  Lap time limits + lap_slicer options
    %  Built via smp_lap_opts_build so this always matches smp_compile_event.
    % ------------------------------------------------------------------
    lap_opts = smp_lap_opts_build(season, track, opts);
    lap_opts.verbose = false;   % lap_slicer's own logging stays off here regardless of
                                % opts.verbose, which controls this function's fprintf output instead.
    fprintf('Lap time limits: %.1fs - %.1fs\n\n', lap_opts.min_lap_time, lap_opts.max_lap_time);
    ...
    for g = 1:n_groups
        ...
        if ~isfield(cache.stats, group_key)
            fprintf('  [INFO] group_key "%s" not in cache.stats yet — will create.\n', group_key);
            cache.stats.(group_key) = struct();
        end
        ...
        try
            ...
            if verbose && ~isempty(custom_fields)
            ...
            % ---- Compute stats for ALL channels (standard + gated + VCH) ----
            all_stat_channels = unique([channels(:); ...
                                        T_gated.CHANNEL_NAME(:); ...
                                        custom_fields(:)]);
            all_stats = lap_stats(laps, all_stat_channels, ...
                struct('operations', {stat_ops}));

            % ---- Build traces (top-N flying laps, same as process_stream) ----
            max_traces_opt = get_opt(opts, 'max_traces', inf);
            flying_mask    = strcmp({laps.lap_type}, 'flying');
            flying_laps    = laps(flying_mask);
            if isempty(flying_laps)
                traces = struct('lap_times', [], 'lap_numbers', [], ...
                                'lap_types', {{}}, 'n_traces', 0);
            else
                [~, sort_idx] = sort([flying_laps.lap_time], 'ascend');
                n_keep   = min(max_traces_opt, numel(flying_laps));
                top_laps = flying_laps(sort_idx(1:n_keep));
                traces   = package_traces(top_laps, channels);
                ...
                fprintf('  Top %d lap times: %s s\n', n_keep, ...
                    strjoin(arrayfun(@(t) sprintf('%.2f', t), traces.lap_times, ...
                                     'UniformOutput', false), '  '));
            end
            clear laps flying_laps;

            % ---- Write everything into cache ----
            cache.stats.(group_key)  = all_stats;
            cache.traces.(group_key) = traces;

            % Backfill manifest: GroupKey + LoadOK for all files in this group
            grp_files_cell = cellstr(grp.files);
            for fi = 1:height(cache.manifest)
                if ismember(cache.manifest.Path{fi}, grp_files_cell)
                    cache.manifest.GroupKey{fi} = group_key;
                    cache.manifest.LoadOK(fi)   = true;
                end
            end

            fprintf('  Stored %d stat channel(s) + %d trace lap(s) in cache.%s\n', ...
                numel(fieldnames(all_stats)), traces.n_traces, group_key);
...
function session = load_and_concat(files, channels_to_extract, verbose, T_gated, load_all, driver_map)
    ...
    EXCEL_FILTERING        = which('filterRequest.xlsx');
...
function groups = groups_from_manifest(manifest, session_filter)
    ...
    % Include all rows that have a resolvable group key (LoadOK filter removed
    % so failed-on-previous-compile entries are re-attempted here).
    has_key  = ~cellfun(@isempty, manifest.GroupKey);

    % For rows with no stored GroupKey, derive one the same way smp_append_stints does.
    % Format: makeValidName(lower('team|driver|session|car'))
    if any(~has_key)
        for ri = find(~has_key)'
            raw = lower(sprintf('%s|%s|%s|%s', ...
                strtrim(manifest.TeamAcronym{ri}), ...
                strtrim(manifest.Driver{ri}), ...
                strtrim(manifest.Session{ri}), ...
                strtrim(manifest.CarNumber{ri})));
            raw = regexprep(raw, '\s+', '_');
            manifest.GroupKey{ri} = matlab.lang.makeValidName(raw);
        end
    end
...
function traces = package_traces(top_laps, channels_to_extract)
    traces = struct();
    if isempty(top_laps), traces.n_traces = 0; return; end
