...
%         % Print
%         if verbose
%             print_xref_drivers_detail(drv_matched, drv_unmatched, result.manifest, 'driverAlias.xlsx');
%             if ~isempty(result.venues)
%                 print_xref('VENUES', ven_matched, ven_unmatched, 'eventAlias.xlsx  [VENUE sheet]');
%             end
%             print_xref('SESSIONS', ses_matched, ses_unmatched, 'eventAlias.xlsx  [SESSION sheet]');
...
% end
% 
% function print_xref_drivers_detail(matched, unmatched, manifest, alias_file_hint)
% % Same as print_xref, but for each UNMATCHED driver, also lists every
% % file (filename + team) that produced that raw driver string — so you
% % can go straight to the source file(s) to add the alias.
%     if nargin < 4, alias_file_hint = 'alias file'; end
%     MAX_FILES_SHOWN = 10;
% 
%     n_total = numel(matched) + numel(unmatched);
%     fprintf('\n--- DRIVERS (%d found, %d matched, %d unmatched) ---\n', ...
%         n_total, numel(matched), numel(unmatched));
% 
%     if ~isempty(matched)
%         fprintf('  Matched:\n');
...
%     if ~isempty(unmatched)
%         fprintf('  *** UNMATCHED — add to %s ***\n', alias_file_hint);
%         manifest_drv_lower = lower(strtrim(manifest.Driver));
%         for i = 1:numel(unmatched)
%             raw = unmatched{i};
%             fprintf('    [!!] %s\n', raw);
% 
%             rows = find(strcmp(manifest_drv_lower, lower(strtrim(raw))));
%             n_rows = numel(rows);
%             n_show = min(n_rows, MAX_FILES_SHOWN);
%             for j = 1:n_show
%                 r = rows(j);
%                 [~, fname, ext] = fileparts(manifest.Path{r});
%                 fprintf('           team=%-10s  file=%s%s\n', ...
%                     manifest.TeamAcronym{r}, fname, ext);
%             end
%             if n_rows > MAX_FILES_SHOWN
%                 fprintf('           ... and %d more file(s)\n', n_rows - MAX_FILES_SHOWN);
%             end
%         end
%     end
% end
...
%     .drivers.matched          cell {raw, canonical, match_method} per matched name
%     .drivers.unmatched        cell of raw strings with no alias match
%     .drivers.unmatched_detail table: RawName, TeamAcronym, Filename, Path
%                                — one row per FILE behind each unmatched name
%     .drivers.n_matched        count
%     .drivers.n_unmatched      count
%     .venues.matched           cell {raw, canonical, match_method} per matched name
%     .venues.unmatched         cell of raw strings with no alias match
%     .venues.unmatched_detail  table: RawName, TeamAcronym, Filename, Path
%     .venues.n_matched         count
%     .venues.n_unmatched       count
%     .sessions.matched         cell {raw, canonical, match_method} per matched name
%     .sessions.unmatched       cell of raw strings with no alias match
%     .sessions.unmatched_detail table: RawName, TeamAcronym, Filename, Path
%     .sessions.n_matched       count
%     .sessions.n_unmatched     count
%     .ok                       true if ALL drivers AND sessions matched
%
...
%   result.sessions                    % paste into eventAlias.xlsx SESSION sheet
%   result.xref.ok                     % true only when drivers AND sessions fully matched
%   result.xref.sessions.unmatched_detail   % table: which file/team produced each bad session name
%   result.xref.drivers.unmatched_detail    % table: which file/team produced each bad driver name
    ...
    for t = 1:numel(teams)
        ...
        for f = 1:numel(files)
            ...
            try
                info = motec_ld_info(fpath, false);
                paths{row}       = fpath;
                team_names{row}  = tm.acronym;
                drivers{row}     = info.driver;
                venues{row}      = info.venue;
    ...
    if ~isempty(alias_file)
        ...
        xref.drivers.n_matched       = numel(drv_matched);
        xref.drivers.n_unmatched     = numel(drv_unmatched);
        xref.drivers.unmatched_detail = build_unmatched_detail(drv_unmatched, result.manifest, 'Driver');

        xref.venues.matched          = ven_matched;
        xref.venues.unmatched        = ven_unmatched;
        xref.venues.n_matched        = numel(ven_matched);
        xref.venues.n_unmatched      = numel(ven_unmatched);
        xref.venues.unmatched_detail = build_unmatched_detail(ven_unmatched, result.manifest, 'Venue');

        xref.sessions.matched          = ses_matched;
        xref.sessions.unmatched        = ses_unmatched;
        xref.sessions.n_matched        = numel(ses_matched);
        xref.sessions.n_unmatched      = numel(ses_unmatched);
        xref.sessions.unmatched_detail = build_unmatched_detail(ses_unmatched, result.manifest, 'Session');
        ...
        % Print
        if verbose
            print_xref_detail('DRIVERS', drv_matched, drv_unmatched, result.manifest, 'Driver', 'driverAlias.xlsx');
            if ~isempty(result.venues)
                print_xref_detail('VENUES', ven_matched, ven_unmatched, result.manifest, 'Venue', 'eventAlias.xlsx  [VENUE sheet]');
            end
            print_xref_detail('SESSIONS', ses_matched, ses_unmatched, result.manifest, 'Session', 'eventAlias.xlsx  [SESSION sheet]');
            print_xref_summary(xref);
        end
...
function detail = build_unmatched_detail(unmatched, manifest, column_name)
% Join each unmatched raw string back to every manifest row that
% produced it, returning a table: RawName, TeamAcronym, Filename, Path.
% Stored in result.xref so it's inspectable after the fact, without
% relying on scrolling back through console output.
    raw_name  = {};
    team      = {};
    filename  = {};
    path_full = {};
    ...
    if isempty(unmatched)
        detail = table(raw_name(:), team(:), filename(:), path_full(:), ...
            'VariableNames', {'RawName','TeamAcronym','Filename','Path'});
        return;
    end

    manifest_col_lower = lower(strtrim(manifest.(column_name)));
    for i = 1:numel(unmatched)
        raw = unmatched{i};
        rows = find(strcmp(manifest_col_lower, lower(strtrim(raw))));
        for j = 1:numel(rows)
            r = rows(j);
            [~, fname, ext] = fileparts(manifest.Path{r});
            raw_name{end+1}  = raw;                      %#ok<AGROW>
            team{end+1}      = manifest.TeamAcronym{r};  %#ok<AGROW>
            filename{end+1}  = [fname ext];               %#ok<AGROW>
            path_full{end+1} = manifest.Path{r};          %#ok<AGROW>
        end
    end
...
function print_xref_detail(label, matched, unmatched, manifest, column_name, alias_file_hint)
% Same as print_xref, but for each UNMATCHED value, also lists every
% file (filename + team) that produced that raw string — so you can go
% straight to the source file(s) to add the alias or investigate.
%
% column_name selects which manifest column to join back against:
% 'Driver', 'Venue', or 'Session'.
    if nargin < 6, alias_file_hint = 'alias file'; end
    MAX_FILES_SHOWN = 10;
