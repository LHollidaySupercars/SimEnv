...
%   sort         'none'|'descend'|'ascend'  sort x-axis by mean   (default: 'none')
%   yLim         [lo hi]             manual y-axis limits         (default: auto)
%   title        string              override figure title        (default: auto)
%   excludeCars  string or cellstr   car numbers to exclude       (default: none)
%   includeCars  string or cellstr   only show these car numbers  (default: all)
    ...
    addParameter(p, 'excludeCars', {});      % car numbers to exclude
    addParameter(p, 'includeCars', {});      % car numbers to include (empty = all)
    parse(p, varargin{:});
    opt = p.Results;
    ...
    if ~isempty(opt.includeCars)
        inc = string(opt.includeCars);
        T   = T(ismember(string(T.car), inc), :);
    end
    if ~isempty(opt.excludeCars)
        exc = string(opt.excludeCars);
        T   = T(~ismember(string(T.car), exc), :);
    end
    ...
    if ~strcmp(opt.sort, 'none')
        ...
        x_col   = 'x_pos';
        x_label = 'Sorted Descending Max Speed By Competitor';
        x_ticks  = [];
        x_labels = {};
    else
        x_col   = 'lap';
    ...
    if ~isempty(opt.title)
        ...
        if strcmp(opt.report, 'top_speed')
            title_str = sprintf('%s — %s   |   %s', ...
                upper('Speed Trap'), strjoin(events, ', '), strjoin(sessions, ' / '));
        else
