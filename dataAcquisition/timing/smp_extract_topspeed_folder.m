function T_all = smp_extract_topspeed_folder(folder_path)
...
%              report_type, identifier, v8sc_name, round, source_file, extracted_at
    ...
    fprintf('════════════════════════════════════════\n');
    fprintf('Done  │  %d/%d PDFs extracted\n', n_ok, numel(pdf_files));
...
function alias = filename_to_session_alias(name)
    ...
    tok = regexp(s, '^(Qualifying\s+Race|Race|Qualifying|Practice|Warm[\s_\-]?Up|Sprint)\s+[A-Z]?(\d+)$', ...
