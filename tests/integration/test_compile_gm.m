classdef test_compile_gm < matlab.unittest.TestCase
% TEST_COMPILE_GM  Integration test for GM/Chev (Anton De Pasquale, car 18) test data.
%
% Expected values:
%   Car:          18
%   Driver:       Anton De Pasquale
%   Manufacturer: Chev / GM
%   Venue:        Albert Park Grand Prix Circuit
%   Session:      Qualifying 6
%   Flying laps:  1
%   Best lap:     ~106.812s  (1:46.812)
%   Channels:     Ground_Speed, C1_Damper_Pos_FL present
%
% Run with:
%   results = runtests('tests/integration/test_compile_gm');
%   table(results)

    properties (Constant)
        TEAM_FOLDER  = 'C:\SimEnv\tests\testData\02_GM'
        BEST_LAP_S   = 106.812
        LAP_TOL_S    = 0.5
        N_FLYING     = 1
    end
    ...
    methods (TestMethodSetup)
        function loadData(tc)
            tc.assumeTrue(isfolder(tc.TEAM_FOLDER), ...
                '02_GM test folder not found — skipping');
    ...
    methods (Test)
        ...
        function testDriverName(tc)
            tc.verifyTrue(contains(lower(tc.info.driver), 'pasquale') || ...
                          contains(lower(tc.info.driver), 'anton'), ...
                sprintf('Expected De Pasquale, got: %s', tc.info.driver));
        end

        function testCarNumber(tc)
            tc.verifyTrue(contains(char(tc.info.vehicle_id), '18'), ...
                sprintf('Expected car 18, got: %s', char(tc.info.vehicle_id)));
        end
