function radiiOffset = rearAArmCompensation(vehicle, manufacturer, axle, shims, varargin)
    ...
    P_UBJ = vehicle.(manufacturer).kinematics.(axle).upperAArm.ballJointCAD;
%             vehicle.ford.kinematics.rear.upperAArm.ballJointCAD
...
%     P_cU =[680.5000, 455.0000, 296.0000];
    ...
    if CAD_ERROR
        shimStack = sum([vehicle.kinematics.rear.camberShims_3127;
                         ...
                         vehicle.kinematics.rear.camberShims_CAD_ERROR]  .*[0,0,1,1]') + vehicle.(manufacturer).kinematics.rear.upperAArm.uprightConnector;
    elseif ~CAD_ERROR
        shimStack = sum([vehicle.kinematics.rear.camberShims_3127;
                         vehicle.kinematics.rear.camberShims_3140;
                         vehicle.kinematics.rear.camberShims_3141]  .*shims') + vehicle.(manufacturer).kinematics.rear.upperAArm.uprightConnector;
    end
