function [uprightToeRod_new, toeRod] = threeSphereDisplacement(vehicle, manufacturer, steeringDisplacement, varargin)

    p = inputParser;
        addRequired(p, 'vehicle');
        addRequired(p, 'manufacturer');
        addRequired(p, 'steeringDisplacement');
        addParameter(p, 'axle', 'rear');
        addParameter(p, 'plotResults', false, @isboolean);


        parse(p,vehicle, manufacturer, steeringDisplacement, varargin{:});
...
% --- Adjustable Parameters ---
    ...
    if ~isvector(steeringDisplacement)
        warning('Displacement was not given as vector displacement, toe rod will fail now')
        return
    end

% --- Extract Points ---
    LBJ                = vehicle.(manufacturer).kinematics.(axle).lowerAArm.ballJoint;
    UBJ                = vehicle.(manufacturer).kinematics.(axle).upperAArm.ballJoint;
    uprightToeRod_old  = vehicle.(manufacturer).kinematics.(axle).lowerAArm.toeRodUpright;
    chassisToeRod_old  = vehicle.(manufacturer).kinematics.(axle).lowerAArm.toeRodChassis;
    ...
    toeRod.deltaError = norm(LBJ - uprightToeRod_old) - norm(LBJ - uprightToeRod_new);
    % --- Plot ---
    if plotResults
        figure; hold on;
