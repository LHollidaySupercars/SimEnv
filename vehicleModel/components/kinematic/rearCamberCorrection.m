function [uprightToeRod_new, toeRod] = rearCamberCorrection(vehicle, manufacturer, varargin)
    ...
    p = inputParser;
        ...
        addParameter(p, 'CAD_ERROR', false, @isboolean)
...
% --- Extract Points ---
    ...
    chassisToUBJ       = vehicle.(manufacturer).kinematics.(axle).upperAArm.fore;
    chassisToLBJ       = vehicle.(manufacturer).kinematics.(axle).lowerAArm.fore;
    % kind of want to run the kinematics file as that willgive me the d hat
    % that I need to be able tod o that
    chassisToUBJ_new   = chassisToeRod_old + steeringDisplacement;
