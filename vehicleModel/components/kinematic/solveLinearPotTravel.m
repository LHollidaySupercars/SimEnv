function vehicle = solveLinearPotTravel(vehicle, varargin)
...
%
% Outputs:
%   vehicle - Updated vehicle structure with Pot travel data
    ...
    addParameter(p, 'maxPotLength', 700, @(x) isnumeric(x) && isscalar(x) && x > 0);
    addParameter(p, 'minPotLength', 200, @(x) isnumeric(x) && isscalar(x) && x > 0);
    ...
    maxPotLength = p.Results.maxPotLength;
    minPotLength = p.Results.minPotLength;
    sweepRange = p.Results.sweepRange;
    numPoints = p.Results.numPoints;
    debug = p.Results.debug;
    doPlotting = p.Results.Plotting;
    
    % Validate Pot length parameters
    if minPotLength >= maxPotLength
        error('minPotLength must be less than maxPotLength');
    end
    ...
    % Vector to be rotated: from lower A-arm fore point to Pot pickup on rocker
    % Translate to origin (lower A-arm fore point becomes origin)
    rockerPotPickup = kin.lowerAArm.potPickupPoint(:)';  % Row vector (1×3)
    vectorToRotate = rockerPotPickup - lowerArmFore;  % (1×3)
    
    % Chassis Pot pickup point (translated to same origin)
    chassisPotPickup = kin.chassis.potPickupPoint(:)';  % Row vector (1×3)
    chassisPickupFromOrigin = chassisPotPickup - lowerArmFore;  % (1×3)
    
    % Pre-allocate output arrays
    rotatedPositions = zeros(numPoints, 3);  % (n×3)
    PotLength = zeros(numPoints, 1);       % (n×1)
    rotationError = zeros(numPoints, 1);      % (n×1)
    
    % Initialize debug figure if needed
    if debug >= 1
        debugFig = figure('Name', 'Pot Travel Debug');
        hold on; grid on; axis equal;
        xlabel('X [mm]'); ylabel('Y [mm]'); zlabel('Z [mm]');
        title(sprintf('%s %s Pot Rotation Debug', upper(manufacturer), upper(axle)));
        view(3);
    end
    
    % Loop through rotation angles and calculate Pot positions
    for i = 1:numPoints
        ...
        % Calculate Pot length (distance from rotated pickup to chassis pickup)
        PotVector = rotatedVec - chassisPickupFromOrigin;
        PotLength(i) = norm(PotVector);
        ...
        if debug >= 1 && mod(i, 50) == 0
            ...
            % Print debug info
            if debug >= 2
                fprintf('Iteration %d: θ = %.2f°, Pot Length = %.2f mm, Error = %.6f mm\n', ...
                        i, rad2deg(theta), PotLength(i), rotationError(i));
            end
        end
    end
    ...
    % Find indices where Pot length is within specified range
    validIndices = find(PotLength >= minPotLength & PotLength <= maxPotLength);
    
    if isempty(validIndices)
        warning('No Pot positions found within specified length range [%.1f, %.1f] mm', ...
                minPotLength, maxPotLength);
        % Find closest approaches
        [~, idxMin] = min(abs(PotLength - minPotLength));
        [~, idxMax] = min(abs(PotLength - maxPotLength));
    ...
    vehicle.(manufacturer).kinematics.(axle).Pot.chassisPickupRot = ...
        rotatedPositions(indexMin:indexMax, :);
    
    vehicle.(manufacturer).kinematics.(axle).Pot.chassisPickupError = ...
        rotationError(indexMin:indexMax);
    
    vehicle.(manufacturer).kinematics.(axle).Pot.displacement = ...
        PotLength(indexMin:indexMax);
    
    vehicle.(manufacturer).kinematics.(axle).Pot.length = ...
        PotLength(indexMin:indexMax) - PotLength(indexMax);
    
    vehicle.(manufacturer).kinematics.(axle).Pot.thetaL = ...
        (thetaSweep(indexMin:indexMax) - datumPosition)';  % Column vector
    [~, indexOfMin] = min(abs(vehicle.(manufacturer).kinematics.(axle).Pot.thetaL));
    vehicle.(manufacturer).kinematics.(axle).Pot.thetaL_0Index = indexOfMin;
    % Optional plotting
    if doPlotting
        plotDamperTravel(thetaSweep, PotLength, rotationError, ...
                        indexMin, indexMax, minPotLength, maxPotLength, ...
                        manufacturer, axle, vehicle, wheelCentre, 'displacement');
                    
       PotLengthFrame = PotLength - PotLength(indexMax);
       minPotLength = minPotLength - PotLength(indexMax)
       maxPotLength = maxPotLength - PotLength(indexMax) 
       plotDamperTravel(thetaSweep, PotLengthFrame, rotationError, ...
                        indexMin, indexMax, minPotLength, maxPotLength, ...
       ...
       % Pot vs Damper plot (requires solveDamperTravel to have run first)
       if isfield(vehicle.(manufacturer).kinematics.(axle), 'damper') && ...
               isfield(vehicle.(manufacturer).kinematics.(axle).damper, 'thetaL')
           damperThetaL      = vehicle.(manufacturer).kinematics.(axle).damper.thetaL;
           damperDisp        = vehicle.(manufacturer).kinematics.(axle).damper.length;
           potThetaL         = vehicle.(manufacturer).kinematics.(axle).Pot.thetaL;
           potDisp           = vehicle.(manufacturer).kinematics.(axle).Pot.length;

           % Interpolate pot onto damper thetaL grid over shared overlap
           overlapMask       = damperThetaL >= min(potThetaL) & damperThetaL <= max(potThetaL);
           damperDispOverlap = damperDisp(overlapMask);
           thetaOverlap      = damperThetaL(overlapMask);
           potDispInterp     = interp1(potThetaL, potDisp, thetaOverlap, 'linear');

           % Instantaneous d(pot)/d(damper) ratio
           pot2damperMR  = diff(potDispInterp) ./ diff(damperDispOverlap);
           damperMidpts  = (damperDispOverlap(1:end-1) + damperDispOverlap(2:end)) / 2;

           figure('Name', sprintf('Pot vs Damper - %s %s', upper(manufacturer), upper(axle)), ...
                  'Position', [50, 50, 1200, 500]);

           subplot(1, 2, 1);
           hold on; grid on;
           plot(damperDispOverlap, potDispInterp, 'b-', 'LineWidth', 2);
           xlabel('Damper Displacement [mm]');
           ylabel('Pot Displacement [mm]');
           title(sprintf('Pot vs Damper Displacement - %s %s', upper(manufacturer), upper(axle)));
           hold off;

           subplot(1, 2, 2);
           hold on; grid on;
           plot(damperMidpts, pot2damperMR, 'r-', 'LineWidth', 2);
           xlabel('Damper Displacement [mm]');
           ylabel('Pot/Damper MR [mm/mm]');
           title(sprintf('Pot:Damper Motion Ratio - %s %s', upper(manufacturer), upper(axle)));
           hold off;
       end
    end
    ...
    if debug >= 1
        ...
        fprintf('Pot length range: %.2f to %.2f mm\n', ...
                min(PotLength(indexMin:indexMax)), max(PotLength(indexMin:indexMax)));
...
function validateVehicleStructure(vehicle, manufacturer, axle)
    ...
    requiredFields = {'lowerAArm', 'chassis', 'rocker'};
    ...
    % Check Pot points
    if ~isfield(kin.chassis, 'potPickupPoint')
        error('chassis must contain "potConnectionPoint" point');
    end
    
    if ~isfield(kin.lowerAArm, 'potPickupPoint')
        error('Pot must contain "chassisPickup" point');
    end
end
...
function plotDamperTravel(thetaSweep, damperLength, rotationError, ...
    ...
    if hasWheelTravel
        ...
        if strcmp(plotType, 'displacement')
            displacement = vehicle.(manufacturer).kinematics.(axle).Pot.displacement;
        elseif strcmp(plotType, 'length')
            displacement = vehicle.(manufacturer).kinematics.(axle).Pot.length;
        end
        ...
        % wheelTravel is on the camber sweep thetaL grid (149 pts).
        % Pot displacement is on the pot thetaL grid (1001 pts trimmed).
        % Interpolate wheelTravel(:,3) onto the pot's thetaL so both arrays align.
        potThetaL    = vehicle.(manufacturer).kinematics.(axle).Pot.thetaL;
        camberThetaL = vehicle.(manufacturer).kinematics.(axle).camberSweep.thetaL;
        wt_z_raw     = wheelTravel(:, 3);
        verticalWheelTravel = interp1(camberThetaL, wt_z_raw, potThetaL, 'linear', 'extrap');
