...
%   vehicle - Updated vehicle structure with damper travel data
    ...
    % Store damper thetaL on damper struct only — do NOT overwrite
    % camberSweep.thetaL which belongs to solveWheelCamber.
    vehicle.(manufacturer).kinematics.(axle).damper.thetaL = ...
        (thetaSweep(indexMin:indexMax) - datumPosition)';  % Column vector
    [~, indexOfMin] = min(abs(vehicle.(manufacturer).kinematics.(axle).damper.thetaL));
    vehicle.(manufacturer).kinematics.(axle).damper.thetaL_0Index = indexOfMin;
    % Optional plotting
    if doPlotting
        plotDamperTravel(thetaSweep, damperLength, rotationError, ...
                        indexMin, indexMax, minDamperLength, maxDamperLength, ...
