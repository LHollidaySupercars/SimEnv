...
TOP_LEVEL_DIR   = 'E:\2026\01_SMP\_TeamData';
...
compile_opts.session_filter = SESSION_FILTER;   % {} = compile all sessions
...
%  Cache is saved AFTER the PPTX regardless of success/failure.
% =========================================================
figs = holdFigs;
...
try
    ...
    pptx_ok = true;
    fprintf('Time to PPTX complete: %.1f minutes (%.0f seconds)\n', ...
        toc(t_script)/60, toc(t_script));
catch ME_pptx
    fprintf('\n[ERROR] PowerPoint export failed: %s\n', ME_pptx.message);
    fprintf('  Figures are still open in MATLAB.\n');
end
...
fprintf('\n=== Total script time: %.1f minutes (%.0f seconds) ===\n', ...
    toc(t_script)/60, toc(t_script));